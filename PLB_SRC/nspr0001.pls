...
.begin patch          1.73
           include    Nseldd.inc
.end patch          1.73
Release    INIT    "1.73"               DLH      .Add Exchange only flag. moved temp files from c:\work to d\users\username
Reldate   Init      "2016 February 11"
.Release    INIT    "1.72"               DLH      .osflag
.Reldate   Init      "2016 January 6"
.Release    INIT    "1.71"               DLH      .Excel 2013 *WindowState=xlMinimized
.Reldate   Init      "2014 January 22"
.RELEASE   INIT      "1.7"       DLH    .change Laser3 to Laser4
.Reldate   Init      "2013 December 12"     
.RELEASE   INIT      "1.6"       ASH    .change exclusive option to allow Order Date suppressing when only using Suppressing Mailer
.Reldate   Init      "2013 September 25"     
...
.Files used
TempName init   "\\nins1\d\user\Username\Suppress.dat"
.TempName2 init  "C:\WORK\Suppress.srt"
TempIName init  "\\nins1\d\user\Username\Suppress.isi|nins1:502                "
...
.end Patch 1.37
.begin Patch 1.73
        SuppressListViewFinal.InsertColumn using "Exch Only",40,8
.end Patch 1.73
...
.START PATCH 1.6 MOVED LOGIC FROM BELOW
          getitem SuppressComboDate1,0,DateParam1
.END PATCH 1.6 MOVED LOGIC FROM BELOW
          if (RptFlag <> C1)   .Regular Suppression OR Exclusive with Secondary Mailers
...
.
.START PATCH 1.6 MOVED LOGIC
.                  getitem SuppressComboDate1,0,DateParam1
.END PATCH 1.6 MOVED LOGIC
...
.Delete any existing Suppression Files
         ...
         pack         TempName  from "\\nins1\d\users\",user,"\Suppress.dat"  
         pack         TempIName  from "\\nins1\d\users\",user,"\Suppress.isi|NINS1:502"  
         Erase        TempName  
         Erase        TempIName  
.        move    "                                        ",APIFileName
.        clear   APIFileName
...
.        prepare TempFile1,TempName,TempIName,C6,"115"
        prepare TempFile1,TempName,TempIName,C6,"117"
.END PATCH 1.5 REPLACED LOGIC
...
.                                read    TempFile1,OLNUM;str6,MLSTNAME,STATUS,PendStat,LCRStat,UNIVERSE,str5,REVDATE,NEWDATE,Managed
                                read    TempFile1,OLNUM;str6,MLSTNAME,STATUS,PendStat,LCRStat,UNIVERSE,str5,REVDATE,NEWDATE,Managed,ExchOnly
.end patch 1.73
.end Patch 1.37
.END PATCH 1.23 REPLACED LOGIC
.END PATCH 1.2 REPLACED LOGIC
...
.                                                                            write   TempFile1,OLNUM;OLNUM,MLSTNAME,STATUS,PendStat,LCRStat,UNIVERSE,str5,REVDATE,NEWDATE,Managed
                                                                            write   TempFile1,OLNUM;OLNUM,MLSTNAME,STATUS,PendStat,LCRStat,UNIVERSE,str5,REVDATE,NEWDATE,Managed,ExchOnly
.end patch 1.73
.end Patch 1.37
.END PATCH 1.23 REPLACED LOGIC
.END PATCH 1.2 REPLACED LOGIC
...
.                                                                          update  TempFile1;OLNUM,MLSTNAME,STATUS,PendStat,LCRStat,UNIVERSE,str5,REVDATE,NEWDATE
                                                                                                              update  TempFile1;OLNUM,MLSTNAME,STATUS,PendStat,LCRStat,UNIVERSE,str5,REVDATE,NEWDATE,Managed,ExchOnly
.end Patch 1.37
.END PATCH 1.23 REPLACED LOGIC
.END PATCH 1.2 REPLACED LOGIC
                                                                                                    endif
...
.                read    TempFile1,SEQ;str6,MLSTNAME,str1,PendStat,LCRStat,UNIVERSE,str5,REVDATE,NEWDATE
                read    TempFile1,SEQ;str6,MLSTNAME,str1,PendStat,LCRStat,UNIVERSE,str5,REVDATE,NEWDATE,Managed,ExchOnly
.end Patch 1.37
.END PATCH 1.23 REPLACED LOGIC
.END PATCH 1.2 REPLACED LOGIC
                until over
...
.end Patch 1.37
.begin Patch 1.73
                    SuppressListViewFinal.SetItemText using N9,ExchOnly,7
.end Patch 1.73
...
.                                                                  write   TempFile1,LSTNUM;LSTNUM,MLSTNAME,STATUS,PendStat,LCRStat,UNIVERSE,str5,REVDATE,NEWDATE,Managed
                                                                  write   TempFile1,LSTNUM;LSTNUM,MLSTNAME,STATUS,PendStat,LCRStat,UNIVERSE,str5,REVDATE,NEWDATE,Managed,eXCHoNLY
.end Patch 1.73
.end Patch 1.37
                                                                      else
                                                                      filepi  1;TempFile1
...
.START PATCH 1.6 ADDED LOGIC
SuppressLoadReport3
        if (FromDate1 = "00000000" AND ToDate1 = "99999999")  .Safeguard...
                return
        endif
        call    SuppressClearScreen
          setitem   SuppressCheckExclusive,0,0
...
.Perform Suppression
          if (ExcFlag = 1)
                    if (FromDate3 = "00000000" AND ToDate3 = "99999999")
                            pack      taskname,"Exclusives Only"
                    else
                            pack      taskname,"Exclusives Only/List Date Select"
                    endif
                    if (DateParam1 = 1)     .Order Dates
                            pack    taskname,taskname," using Order Dates"
                    elseif (DateParam1 = 2) .Mail Dates
                            pack    taskname,taskname," using Mail Dates"
                    elseif (DateParam1 = 3) .Return Dates
                            pack    taskname,taskname," using Return Dates"
                    endif
                    setitem SuppressStatPrimary2,0,taskname
          else
                    setitem SuppressStatPrimary2,0,"List Date Select"
          endif
        move    C0,howmany    .Counter used to determine how many records read
...
.
                                                        ...
                                                        loop
                                                                ...
                                                                move    "SuppressLoad2-NORDKG",Location
                                                                call    NORDKG
                                                        repeat
                                                        if (ExcDateFlag = Yes)
                                                                read    TempFile1,LSTNUM;;
...
.begin Patch 1.73
          pack      str11,"F",str9
          setprop sheet.range(str11),*Value="Exchange"
.end Patch 1.73
...
.end Patch 1.37
.begin Patch 1.73
                SuppressListViewFinal.GetItemText giving exchOnly using result,C7
.end Patch 1.73
...
.begin Patch 1.73
                    pack      str10,"F",str9
                setprop sheet.range(str10),*Value=ExchOnly,*HorizontalAlignment=AlignCenter
.end Patch 1.73
                add     C1,N10
        repeat
        move    N10,str10
...
SuppressOpenPrintFile
        if (CNTPRINT = 1 or Cntprint = 2)
                if (osflag >= c6)         .Vista or above
                        PRTOPEN prfile,"@Laser4 Legal","FAXFILE.PRN"
.                elseif (osflag = c1)         .win 95 98
.                        PRTOPEN prfile,"@Laser4","FAXFILE.PRN"
.                elseif (osflag = c6)         .XP
.                        PRTOPEN prfile,"@Laser4 On NINS2","FAXFILE.PRN"
                else   .         .Don't know prompt for printer
                        PRTOPEN prfile,"@","FAXFILE.PRN"
                endif
.        elseif (CNTPRINT = 2)  .Laser4
.                if (osflag = c2)         .nt
.                        PRTOPEN prfile,"@\\NINS2\laser4","FAXFILE.PRN"
.                elseif (osflag = c6)         .XP
.                        PRTOPEN prfile,"@Laser4 Legal on NINS2","FAXFILE.PRN"
.                elseif (osflag = c1)         .win 95 98
.                        PRTOPEN prfile,"@Laser4 Legal","FAXFILE.PRN"
...
.                        PRTOPEN prfile,"@Laser2","FAXFILE.PRN"
        ...
        elseif (CNTPRINT = 5)     .Susan
                if (osflag >= c6)       
                      PRTOPEN prfile,"Kyocera FS-1030D",Str45
...
.                        PRTOPEN prfile,"@KYOCERAS","FAXFILE.PRN"
        ...
        elseif (CNTPRINT = 7)     .DH
                if (osflag >= c6)         .nt +
                        PRTOPEN prfile,"@Kyocera FS-1030D KX","FAXFILE.PRN"
