...
.START PATCH 3.79.5 ADDED LOGIC
	include	nspe3dd.inc
.END PATCH 3.79.5 ADDED LOGIC
Release	init	"3.83"	 .        DLH	.Allow Brokerage to place 'Pending' orders
Reldate	Init	"15 July 2008"
.Release	init	"3.82"	 .        DLH	.Allow security mode fixord to ignore some exchange verifications
.Reldate	Init	"12 June 2008"
.Release	init	"3.81"	 .        DLH	Block GOP on exchange order - show shipping qty on screen one if avail
.Reldate	Init	"15 April 2008"
.Release	init	"3.80"	 .        DLH	Superfixordok flag
.Reldate	Init	"06Nov2007"
.Release	init	"3.79.9"	 .        JD replaced users revtypes/listmgmt vars
.Reldate	Init	"22Oct2007"
.Release	init	"3.79.8"	 .DLH More PLI / LM
.Reldate	Init	"27Sep2007"
.Release	init	"3.79.7"	 .DLH replace all uses of ErrMessg - info was truncated replaced with SendMail
.Reldate	Init	"20Sep2007"

.Release	init	"3.79.6"	 DLH PLI Inclusion additional
.Reldate	Init	"13Aug2007"
.Release	init	"3.79.5"	 ASH PLI Inclusion
.		Patch Includes following:
.			1)	Inclusion of New PLI Special Instruction File
.			2)	Inclusion of New OCOMPID/OCOMPID2 Logic
.			3)	Removal of DH OCOMPID/OCOMPID2 Logic
.			4)	Test logic for Report Window on Screen 5
.				to allow user interaction for Corporate logo.
.Reldate	Init	"16Jun2007"
...
.revtyps	init	"DH DM DMB JD "	     ALLOWED REVISION ?
.30 OCt 2007
.revtyps	init	"DH JD JL "	     ALLOWED REVISION ?
.added JG & SK
revtyps	init	"DH JD JL SK JG"	     ALLOWED REVISION ?
.30 OCt 2007
.End patch 3.79.9
.start patch 3.79.9
.ListMgmt init	"SK AA KE JG JF"	.Used to determine List	Managment
ListMgmt init	"SK AC RG JG SL cf"	.Used to determine List	Managment
.end patch   3.79.9
Carr	init	0x7f
...
FromDate dim	8
ToDate	dim	8
.START PATCH 3.79.5 ADDED TEST LOGIC
.CorpFlag form	1
.END PATCH 3.79.5 ADDED TEST LOGIC
.
date	dim	8
.Vars used to determine	MailerStatSalesMssg
.salesper dim	15               .you are kidding me   DLH 02/06/08
salesper dim	20
sqty	form	9
sentry	form	5
...
.End PATCH 3.79.2
.START PATCH 3.79.5
HoldELSTCDE	DIM	1		.Save Datacard Exclusive Byte
HoldOSALES	dim	2		.Save Salesperson code
.END PATCH 3.79.5
...
.END PATCH 3.72 ADDED LOGIC
Nord001a plform	Nord001a                           .screen 1
...
.END PATCH 3.72.4 ADDED LOGIC
Nord001c plform	\\nts0\c\library\plb_src\Nord001c
...
.START PATCH 3.75 TEMP PATCH FOR TESTING!!!!
NordMSK1 plform	\\nts0\c\library\plb_src\NordMSK1
NordMSK2 plform	NordMSK2
NordMSK3 plform	NordMSK3
Nord001d plform	\\nts0\c\library\plb_src\Nord001d
...
.	ALERT	PLAIN,"PORT HARD-coded!!",result
.	move	"112",PORTN
	move	C0,NUSEFLD
	move	C1,NUSEPATH
	move	PORTN,NUSEFLD
...
.START PATCH 3.79.5 ADDED TEST LOGIC
.	create	Report2;StatTextBoxes(4)=90:110:10:110,"Corporate Logo","'>MS Sans Serif'(8)"
.	create	Report2;ComboBoxes(3)=90:111:120:310,"",";N)ames in the News;)Pacific Lists"
...
.	listins	ObjectColl,StatTextBoxes(2),StatTextBoxes(3),ComboBoxes(1),ComboBoxes(2),StatTextBoxes(4),ComboBoxes(3)
..
..Establish default Corporate Logo, which user can change
.	move	C0,NUSEFLD
.	move	C1,NUSEPATH
.	move	PORTN,NUSEFLD
.	rep	zfill,NUSEFLD
.	call	NUSEKEY
.	if (NUSECOMP = "P")
.		setitem	ComboBoxes(3),0,C2
.	else
.		setitem	ComboBoxes(3),0,C1
.	endif
.END PATCH 3.79.5 ADDED TEST LOGIC
...
.Who is	doing an update, and what are they trying to update?!?
			move	"This is a message from	NORDTEST",MailSubjct
			clear	Mailbody
			append	INITS,mailbody
			append	" tried	to update XREF file.",mailbody
			Append	CRLF,Mailbody
			append	"List: ",Mailbody
			append	OLNUM,Mailbody
			Append	CRLF,Mailbody
			append	"Old Mailer: ",Mailbody
			append	NXRFMLR,Mailbody
			Append	CRLF,Mailbody
			append	"New Mailer: ",Mailbody
			append	TEMMLR,Mailbody
			Append	CRLF,Mailbody
			append	"LR: ",Mailbody
...
SecurityGo1
	...
	if (NPASFLD <> "(COSMO")
		return
	Else
	Move	yes,SuperFixordOK
	endif
.............................
SuperFixordOK
	alert	note,"FIXORD mode permitted!",result
.	 SNDOPEN sfile,"\\nts1\d\users\aharkin\ohmigod.wav"
.	 SNDPLAY sfile
.	 SNDCLOSE sfile
...
.begin patch 3.81   .be default - hide
	setprop	NORD001AStatShpQty,Visible=0
	setItem	NORD001AEditShpQty,0,""
	setprop	NORD001AEditShpQty,Visible=0
.end patch 3.81
...
.	if (((N2 > 1) AND (N2 <	5)) AND	(ELSTCDE = "C"))
	if (((N2 > 1) AND (N2 <	5)) AND	(ELSTCDE = "C" OR ELSTCDE = "P"))
.end patch 3.79.6
		setprop	Nord001bComboNINGuar,enabled=0,bgcolor=grey
	else
...
.begin patch 3.79.6
	...
	Elseif	(Elstcde = "P")
	move	C2,OSALES10
	move	c7,OSALES		.List Management
.Begin patch 01/18/2008  the big test
	move	"17",str2	Joey Gamache
.	move	"84",str2	Sarah Lewin
.end patch 01/18/2008  the big test
	endif	
.end patch 3.79.6
	call	OrderLoadSalesperson using C1
	setitem	Nord001bEditContDate,0,""
...
.Test DH 04/16/08
.Nope have not read the datacard if want this to work have to insert a read :(
 	If	(ELstCde = "C")	
               setprop        Nord001AStatListName,FGColor=Blue
.               setprop        Nord001AStatListName,BGColor=Blue
               setprop        Nord001CStatListName,FGColor=Blue
.               setprop        Nord001CStatListName,BGColor=Blue
               setprop        Nord001bStatListName,FGColor=Blue
.               setprop        Nord001BStatListName,BGColor=Blue
	Elseif	(ELstCDE ="P")
               setprop        Nord001AStatListName,FGColor=Green
               setprop        Nord001CStatListName,FGColor=Green
               setprop        Nord001bStatListName,FGColor=Green
.               setprop        Nord001AStatListName,BGColor=grey
.               setprop        Nord001BStatListName,BGColor=grey
.               setprop        Nord001CStatListName,BGColor=grey

               else
               setprop        Nord001AStatListName,FGColor=BlACK
               setprop        Nord001CStatListName,FGColor=BlACK
               setprop        Nord001bStatListName,FGColor=BlACK
.               setprop        Nord001AStatListName,BGColor=grey
.               setprop        Nord001BStatListName,BGColor=grey
.               setprop        Nord001CStatListName,BGColor=grey
               ENDIF
.End Test DH 04/16/08
...
.END PATCH 3.72.9 ADDED LOGIC
.START PATCH 3.79.5 ADDED LOGIC
		type	OLRN	//PLI records are prefaced with ALPHA characters
		if equal
.END PATCH 3.79.5 ADDED LOGIC
			pack	NSPEFLD,OLRN
			rep	zfill,NSPEFLD
...
.
			...
			setitem	nord001CEditSpecial,0,DESC005
			setitem	nord001CEditSpecial1,0,""
			setitem	Nord001AStatExchangeMssg,0,""
		endif
.END PATCH 3.79.5 ADDED LOGIC
...
.ListView #1
	...
	Nord001DListView.SetItemText using N9,OQTY,3
.START PATCH REMOVED LOGIC 3.79.5
..begin patch 3.79.2
.	If	(NewFlag = yes)
.		Move	CompExcl,OCompID
.	endif
..end patch 3.79.2
.END PATCH REMOVED LOGIC 3.79.5
 	if (OELCODE = "2" | OELCODE = "3")
		if (ORENT = "1")  .LCR Rental
			move	"Exc/Rent",str10
		else
...
.begin patch 3.81
	...
	setprop	NORD001AStatShpQty,Visible=0
	setprop	NORD001AEditShpQty,Visible=0
	setitem	Nord001GEditShipQty,0," "
	else
	setItem	NORD001AStatShpQty,0,"Shipped Qty"
	setprop	NORD001AStatShpQty,fgcolor=RED
	setprop	NORD001AStatShpQty,Visible=1
	setprop	NORD001AEditShpQty,Visible=1
	call	FormatNumeric using Squant,str11
	setitem	Nord001AEditShpQty,0,Str11
	setitem	Nord001GEditShipQty,0,Str11
	endif
.end patch 3.81
...
.END PATCH 3.72.9 ADDED LOGIC
.START PATCH 3.79.5 ADDED LOGIC
			type	STATLR	//PLI records are prefaced with ALPHA characters
			if equal
.END PATCH 3.79.5 ADDED LOGIC
				pack	NSPEFLD,STATLR
				rep	zfill,NSPEFLD
...
.		endif
..END PATCH 3.79.2
		Move	CompExcl,HoldCompExcl
.END PATCH REPLACED 3.79.5
		reset	badstat
		scan	MSTAT,badstat
		if equal
...
.begin patch 3.83 DH 08/06/08
.		if (str2 = "06"	| (mod = 5 | mod = 6))	      .List Management
		if (str2 = "06" | str2 = "27" | str2 = "28"	| mod = 5 )	      .List Management 
.begin patch 3.83 DH 08/06/08
...
.START PATCH ADDED LOGIC 3.79.5
		move	ELSTCDE,HoldELSTCDE
.END PATCH ADDED LOGIC 3.79.5
...
.			elseif (ELSTCDE	<> "C")	 .OUTSIDE LIST
			elseif (ELSTCDE <> "C" & ELSTCDE <> "P")	 .OUTSIDE LIST
.end patch 3.79.6
				getitem	Nord001bComboCaller,0,N2
				if (N2 <> C1)
					clear	taskname
...
.begin patch 3.79.6
.	elseif (ELSTCDE	= "C")	.EXCLUSIVE LIST
		elseif (ELSTCDE = "C" OR ELSTCDE = "P")	.EXCLUSIVE LIST
.end patch 3.79.6
			if (mod = 3 OR mod = 7 OR mod =	5 OR mod = 6)
.				 if ((mod = 3 OR mod = 7) AND NORD5STAT	<> "04")
.					 alert	 plain,"This List is EXCLUSIVE,	OK?",result
...
.START PATCH 3.79.5 REPLACED LOGIC
...
......................................
	...
	if equal
		...
		getitem	nord001CEditSpecial,0,DESC005
		count	N7,DESC005
		if (N7 > 0 AND NewFlag = YES)
			move	YES,SpecFlag2
		endif
	endif
.END PATCH 3.79.5 REPLACED LOGIC
...
.VERIFY	SALESPERSON
	getitem	Nord001bEditSales,0,str2
.patch 3.83 - allow brokerage to do Pending orders
.	if (mod	= 5 | mod = 6)
.		move	"06",str2
...
.	if (str2 > "22")
	if (str2 > "35")
		move	B1,OSALES10
		move	B1,OSALES
	elseif (str2 = "06" or str2 = "27" or str2 = "28")		.List Management
.START PATCH 3.6 REPLACED LOGIC
.		if (mod	<> 3 AND (mod <> 7 OR (mod = 7 AND NORD5STAT = "04")))
		if (mod <> 8 AND mod <> 3 AND (mod <> 7 OR (mod = 7 AND NORD5STAT = "04")))
.END PATCH 3.6 REPLACED LOGIC
...
.	if (str2 = "06" & (mod = 5 | mod = 6 | (mod = 7	& NORD5STAT = "04"))) .Pending Order OR	LCR being turned into Live Order
	if ((str2 = "06" | str2 = "27" | str2 = "28") & (mod = 5 | mod = 6 | (mod = 7 & NORD5STAT = "04"))) .Pending Order OR	LCR being turned into Live Order
.end patch 3.79.8	
...
.VERIFY COMPANY IDS
drewtester
	//.New, LCR/Pending ->Live, Change in Mlr/List. Change in OSALES10/OSALES
	pack	str2,OSALES10,OSALES
	if (NewFlag = YES | OMLRNUM <> HoldMlr | OLNUM <> HoldList | str2 <> HoldOSALES | (mod = 6 & NORD4STAT = "08") | (mod = 7 & NORD5STAT = "04"))
		clear	OCOMPID
		clear	OCOMPID2
.begin patch 9999
.		if (OSALES10 = "0" & OSALES = "6")	.List Management
			if (HoldELSTCDE = "P")
				move	"P",OCOMPID2
			Elseif (HoldELSTCDE = "C")				
				move	"N",OCOMPID2
			endif
.		endif					.Brokerage
.end patch 9999
...
.begin patch 3.79.7	
	if (str2 <> "06" & Str2 <> "27" & str2 <> "28" AND mod <> C5 AND (mod	<> C6 OR (mod =	6 AND NORD4STAT	= "08"))) .Pending
...
.	 if (mod = 3 OR	(mod = 7 AND NORD5STAT <> "04"))
	if ((mod = 3 |	(mod = 7 & NORD5STAT <> "04")) | (mod = 5 OR (mod = 6 & NORD4STAT <> "08")))
...
.   Set	the text message that is send with the attachments
		...
		APPEND	crlf,MailBody
		Append	"Subroutine Save3",MailBody
		APPEND	crlf,MailBody
		Reset	MailBody		
.		move	"This is an error message",SmtpTextMessage(1)	Array <Text message >
.		move	taskname,SmtpTextMessage(2)   Array <Text message >
...
.		move	"3",SmtpTextIndexLast				    Index to last entry	in TextMessage array
.		call	errmesg
		pack	Mailto from User,"@nincal.com,INformationservices@nincal.com"
		pack	Mailfrom from User,"@nincal.com"
		call	SendMail
.end patch PLI 3.79.7 DLH
...
.START PATCH 3.76 REPLACED LOGIC - TEMPORARY LOGIC
.		move	NXRFMLR,TEMMLR
.START PATCH REMOVED 3.79.5
..begin patch 2.79.2
.		MOve	CompExcl,HoldCompExcl		.Save Mlr excl byte
..end patch 2.79.2
.END PATCH REMOVED 3.79.5
...
.begin patch November 13 2007
.	 append	 " prin=",taskname
	 append	 " pa=",taskname
.end patch November 13 2007
	 move	 CNTPRINT,str2
...
.begin patch 3.81      ... Do not allow GOP on Exchange only order
		Sub 	N9,N9
		Move	Oexqty,N9              .if not equal it is a split rent/exchange
		if	(OELCODE = "1" or ((OELCODE = "2" or OELCODE = "3")  & N9 <> c0))
...
.START PATCH 3.79.5 ADDED LOGIC
		...
		rep	zfill,NSPE3FLD
		move	C1,NSPE3LOCK
		move	"XCANCEX-NSPE3DEL",Location
		pack	KeyLocation,"Key: ",NSPE3FLD
		call	NSPE3DEL
		if (DESC005 <> "")		.There are special instructions
			move	C1,NSPE3LOCK
			move	"XCANCEX-NSPE3WRT",Location
			call	NSPE3WRT
			IF (LRINIT = 1)
			move	"NINSPEC3 - UPDATECANCELSPECIAL",str45
			call	OrderWriteLRFile using str45
			ENDIF
		endif
	endif
...
.
	move	"This is a message from	NORDTEST",MailSubjct
	Clear	Mailbody
	Append	"Key value and LR value	do not match!",Mailbody
	append	crlf,Mailbody
	Append	"Key: ",Mailbody
...
.   Set	the text message that is send with the attachments
		...
		Append	CRLF,MailBody
		Append	"Subroutine OrderUpdateFiles",MailBody
		Append	CRLF,MailBody
		Reset	MailBody
.		Move	"This is an error message",SmtpTextMessage(1)	Array <Text message >
...
.   Set	the text message that is send with the attachments
		Clear	MailBOdy
		Append	"NORDAIMA,INSERTA-Unable to Update NINORD AamKey!!!",Mailbody
		Append	CRLF,MailBody
		append	"KEY:  ",MailBody
		append	NORDFLD,MailBody
		Append	CRLF,MailBody
		append	"AKEY:	",MailBOdy
		append	NORDFLD1,MailBOdy
		append	COMMA,MailBOdy
		append	NORDFLD2,MailBOdy
		append	COMMA,MailBOdy
		append	NORDFLD3,MailBOdy
		append	COMMA,MailBOdy
		append	NORDFLD4,MailBOdy
		Append	CRLF,MailBody
		Reset	MailBOdy		
...
.   Set	the text message that is send with the attachments
				Clear	MailBody
				Append	"NORDKGA-Unable	to Update NINORD AamKey!!!",MailBody
				Append	CRLF,MailBody
				append	"KEY:  ",MailBOdy
				append	NORDFLD,MailBOdy
				Append	CRLF,MailBody
...
.	endif
			...
			rep	zfill,NSPE3FLD
			move	C1,NSPE3LOCK
			move	"Save-NSPE3DEL",Location
			pack	KeyLocation,"Key: ",NSPE3FLD
			call	NSPE3DEL
			if (SpecFlag = YES OR DESC005 <> "")		  .There are special instructions
				move	OLRN,NSPELR3
				move	C1,NSPE3LOCK
				move	"Save-NSPE3WRT",Location
				pack	KeyLocation,"Key: ",NSPE3FLD
				call	NSPE3WRT
				IF (LRINIT = 1)
				move	"NINSPEC3 - WRTOTHER",str45
				call	OrderWriteLRFile using str45
				ENDIF
			endif
...
.Opportunity to update PLI Instructions
.END PATCH 3.79.5 ADDED LOGIC
...
.		move	"This is a Error e-mail	from Nord0001",SmtpSubject Subject
		move	"This is a Error e-mail	from Nord0001",MailSubjct
.   Set	the text message that is send with the attachments
		Clear	MailBody
		Append	"This is an error message",MailBody
		Append	CRLF,Mailbody
		Append	"WRITDMC Error",MailBody
		Append	CRLF,Mailbody
...
.				else
					append	"!\\nts0\c\apps\winbatch\butil job=HOTORD1 ",TASKNAME
.				endif
				append	" infile=",taskname
				append	str6,taskname
...
.END PATCH 3.50 REPLACED LOGIC
		...
		loop
			...
			clear	taskname
				append	"!\\nts0\c\apps\winbatch\butil job=HOTORDSND ",taskname
.				append	"!\\nts0\c\apps\winbatch\butil job=HOTORDFAX ",TASKNAME
			...
			append	" B=",TASKNAME
			append	user,TASKNAME
.PRINTER NOT REALLY NEC WE ARE GOING TO CC REQUESTOR
			 append	 " pa=",taskname
			if (N4 = 1)
				move	"A",str2
			else
...
OrderApproveCheck
	...
	getprop	Nord001ACheckMode,enabled=result
.Must be in Inquiry mode AND Order cannot be:  Pending/LCR/Cancelled/CancelledBilled AND cannot	be in Search mode!
	if (result = 0 AND OSTAT <> "p" & OSTAT <> "x" & NewFlag <>	"S" & STAT <> "l" & OSTAT <> "z" & OSTAT <> "X" & OSTAT <> "Q")
		if (ORPCODE = "F")		.Order must be in NINPRINT and not approved!
			setitem	NORDMSK1CheckApprove,0,0
...
.   Set	the text message that is send with the attachments
				...
				Append	taskname,Mailbody
				Append	CRLF,MailBody
				Append	"Subroutine OrderMoveCampaign",Mailbody
				Append	CRLF,MailBody
				Reset	MailBody				
.				move	"This is an error message",SmtpTextMessage(1)	Array <Text message >
.				move	taskname,SmtpTextMessage(2)   Array <Text message >
...
.   Set	the text message that is sent with the attachments
			...
			Append	taskname,MailBody
			Append	CRLF,MailBody
			Append	"Subroutine OrderMoveLOLCampaign",MailBody
			Append	CRLF,MailBody
			reset	MailBody			
.			move	"This is an error message",SmtpTextMessage(1)	Array <Text message >
.			move	taskname,SmtpTextMessage(2)   Array <Text message >
...
.   Set	the text message that is send with the attachments
			...
			Append	taskname,MailBody
			Append	CRLF,MailBody
			Append	"Subroutine OrderMoveLOLCampaign2",MailBody
			Append	CRLF,MailBody
			reset	MailBody
.			move	"This is an error message",SmtpTextMessage(1)	Array <Text message >
.			move	taskname,SmtpTextMessage(2)   Array <Text message >
...
.	move	"This is an Error e-mail from Nordtest",SmtpSubject Subject
	move	"This is an Error e-mail from Nordtest",MailSubjct
.   Set	the text message that is send with the attachments
	clear	Mailbody
	append	LocMssg,Mailbody
	append	CRLF,Mailbody
...
.   Set	the text message that is send with the attachments
	Clear	Mailbody
	append	"No Record Found in Exchange File",Mailbody
	append	CRLF,Mailbody
	Append	ErrMssg,Mailbody
...
.	move	"Exchange is a Duplicate!!",SmtpTextMessage(1)	 Array <Text message >
	Clear	Mailbody
	append	"Exchange is a Duplicate!!",Mailbody
	append	CRLF,Mailbody
	append	"Error = ",Mailbody
	Append	Error,Mailbody
	append	CRLF,Mailbody
	append	"Key = ",Mailbody
	Append	Exkey,Mailbody
	reset	Mailbody
.	move	"Exchange is a Duplicate!!",SmtpTextMessage(1)	 Array <Text message >
.	move	ErrMssg,SmtpTextMessage(2)   Array <Text message >
...
.   Set	the text message that is send with the attachments
	...
	Append	error,Mailbody
	Append	CRLF,MailBody
	Append	"Interupt Error",Mailbody
	reset	Mailbody	
.	move	"This is an error message",SmtpTextMessage(1)	.Array <Text message >
.	move	error,SmtpTextMessage(2)			.Array <Text message >
