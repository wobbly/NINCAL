. ............................................................................
.NINP29EDIT - PRE CHECK REGISTER EDIT PROGRAM
...............................................................................
.
PC       EQU       0
         INC       COMMON.inc
...
Release   Init      "3.1"     DLH       Add Escrow stop date logic.
Reldate   Init      "2016 February 25"
.Release   Init      "3.0"     DLH       turn off Inter comp transfer.
..No code changes should be necesary CEXTCD value of "I" no longer possible
.Reldate   Init      "12 November 2009"
.release   init     "2.99"           JD  Moved over "excl" var
.Reldate   INit      "20May2009"
.release   init     "2.98"          DLH Close of PL account
.Reldate   INit      "25Feb2009"
.release   init     "2.97"          DLH Escrow file correction
.Reldate  INit      "05Mar2008"
.release   init     "2.96"          DLH 2008January28 Clarify Inter comp transfers regarding escrows
.Reldate  INit      "28Jan2008"
.release    init       "2.95"           DLH Give total payables figure
.Reldate  INit      "18Dec2007"
.release    init       "2.93"           DLH 26Oct2007  merge JD & DH PLi & other
...
.         if        over
.
.begin patch 2.93 --- moved to after adjustments
         move      c2 to nescpath
...
.begin patch 2.93 --- moved to after adjustments
..begin patch 2.9
..check if ap3 or xninc > 0 and external code is not "I" for 2nd pass do not do the escrow         
.         move      no to escflag
...
.SkipEscrow         
..end patch 2.9
.end patch 2.93 --- moved to after adjustments
.
...
.begin patch 3.35   ---- we don't write ourselves checks the money is already in the bank
          reset     runcodes
          MOve      No,NoCheckFlag
.Begin  if there is AP1 we don't care if it is a runcharge we should not be trying to cut ourselves a check  2014 Dec 10 DLH          
.          if        (COmpany <> "2" & Lon = "0033")
          if        (COmpany <> "2" & Lon = "0033" & AP1 > c0)
.          scan      Olnum,Runcodes
.                    IF        not equal
                    MOVe      yes,NoCheckFlag
.                    endif
.          Elseif    (COmpany = "2" & Lon = "1490")
          Elseif    (COmpany = "2" & Lon = "1490" & AP1 > c0)
.          scan      Olnum,Runcodes
.                    IF        not equal
.                    MOVe      yes,NoCheckFlag
.                    endif
.end  if there is AP1 we don't care if it is a runcharge we should not be trying to cut ourselves a check  2014 Dec 10 DLH          
          endif
.end patch 3.35
.
         MOVE      YES TO SUBPPSW
...
.end patch 2.97
         ...
         move      no to escflag
          ...
          Goto      CheckEscrow
.begin patch 2.96
          ElseIf    ((AP3 <> 0 or XNinc <> 0) & CEXTCD  <> "I")      We have an IntraCorp transaction/transfer
          Goto      SkipEscrow
.end patch 2.96
          Elseif    ((AP3 <= c0 | Xninc <= c0) and CEXTCD  <> "I")
          Goto      CheckEscrow
...
.begin patch 2.92
          ...
          PRINT               *1,MLRN,"/",COBN,*12,LRN:
                    ...
                    *79,M$XNINC:
.begin patch 2.99
                    *N,*6,LOINVN,*20,O1DES,*66,excl,M$AP3,*N
;                    *N,*6,LOINVN,*20,O1DES,*64,excl,M$AP3,*N
.end patch 2.99
          Else
                    If        (CEXTCD = "I")
                    Move      Ap3,CodeIARAMT
...
.begin patch 2.94
.                             PRINT     *1,MLRN,"/",COBN,*12,LRN:
.                             *26,"Pacific Lists Inc.",M$AR,*64,M$AP3," ":
.                             *79,M$XNINC:
.                             *N,1,"** A/P1 above will be in check to Pacific Lists - PLease process this LR again ":
.                             "for payment to the List owner"
.                             ADD       C2 TO LINES
                              PRINT     *1,MLRN,"/",COBN,*12,LRN:
                                        *26,"Pacific Lists Inc.",M$AR,*64,M$Trans:
                                        *n,*64,M$AP3," + ":
                                         *79,M$XNINC," + ",*91,M$AP1:
.begin patch 2.98
.                                        *N,1,"** A/P1 above will be in check to Pacific Lists - PLease process this LR again ":
.                                        "for payment to the List owner"
                                        hpbon:
                                        *N,1,"** A/P1 above is InterCompany Trasfer - Pacific Lists - PROCESS this LR again ":
...
.lr & NIN to to PLI Generally 20%  as broker - cut check to PLI
                              PRINT     *1,MLRN,"/",COBN,*12,LRN:
                                        *26,"Names in the News",M$AR,*64,M$Trans:
                                        *n,*64,M$AP3," + ":
                                         *79,M$XNINC," + ",*91,M$AP1:
.begin patch 2.98
.                                        *N,1,"** A/P1 above will be in check to Names in the News - PLease process this LR again ":
.                                        "for payment to the List owner"
                                        hpbon:
                                        *N,1,"** A/P1 above is INterCOmpany Transfer to Names in the News - Process this LR again ":
                                        "for payment to the List owner",hpboff
.en patch 2.98
                              ADD       C3 TO LINES
                              endif
.Testing            
.Testing turned back on again JD/DH 1/4/08!!!!!!            
                    Sub       Ap1,TotAP1
...
.begin patch 2.99
                    *N,*6,LOINVN,*20,O1DES,*66,excl,M$AP3,*N
;                    *N,*6,LOINVN,*20,O1DES,excl,M$AP3,*N
.end patch 2.99
                    endif
          endif                   
...
.begin patch 2.99
                   *N,*6,LOINVN,*20,O1DES,*66,excl,M$AP3,hpboff
;                   *N,*6,LOINVN,*20,O1DES,excl,M$AP3,hpboff
.end patch 2.99
         move      yes to duprflag
         endif
.begin patch 2.95       .move code
.begin patch 2.9
...
.                   *N,*6,LOINVN,*20,O1DES,hpboff;
.begin patch 2.99
                   *N,*6,LOINVN,*20,O1DES,*66,excl,hpboff;
...
.       
         ...
         cmatch    yes to negflag
         if        not equal          
         PRINT     *N,*6,LOINVN,*20,O1DES,b1,"DIFF: ",diff,*flush:
                    *79,M$XNINC,*N
         else
         PRINT     *N,*6,LOINVN,*20,O1DES,b1,hpbon,"NEGATIVE: ",apamt,hpboff,*flush:
                    *79,M$XNINC,*N
...
.         if         not equal
          If        (totar+transdiff <> NCTRAMT2 & TotAr <> NCTRAMT2)
         move       yes to errflag
         endif
.end patch 2.93         
.end patch 2.85
...
.                   *N,*62,MT$AP2,*77,MT$NINC,*103,MT$STAX,*124,MT$POST:
                   *N,*62,MT$AP2,*77,MT$NINC,*89,MT$LRINC3," *",*103,MT$STAX,*124,MT$POST:
.begin patch 2.9                   
                   *N,*62,MT$AP3,*77,MT$XNINC
.end patch 2.9
         ADD       TOTAP1 TO TOTAP
         ADD       TOTAP2 TO TOTAP
.begin patch 2.95
...
.         PRINT     *26,"TOTAL ACCOUNTS PAYABLE",*62,totmask
         ...
         PRINT     *26,"TOTAL ESCROW APPLIED TO MOA",*62,MT$esc
         PRINT     *26,"TOTAL NEGATIVE AP'S",*62,MT$NEG
.begin patch 2.9
...
.end patch 2.9
.begin patch 2.95
          Calc      TotCheck=(TotAp+TotTrans)
.         Calc      TotCheck=(TotAp+totescro+TotTrans-totap3)
          MOVE                MASK92b,MT$Checks
          edit      Totcheck,MT$Checks
.begin patch 2.98
.          Print     *26,"G Total Checks:",*62,MT$Checks
          Print     *26,"G Total Payables:",*62,MT$Checks
.end patch 2.98
.end patch 2.95
...
.begin patch 2.85
         ...
         print     *1,hpbon,hpunon,"*******************AR TOTAL DOES NOT MATCH CASH PLEASE RE-RUN CASH********************":
                           *n,*26,"REGISTER TOTAL",*48,MT$AR:
                 *n,*26,"Control Total",*48,NCTRAMT2:       
                 *n,*26,"Transfer Total",*48,transdiff:     
                   hpboff,hpunoff
         endif
.end patch 2.85
         GOTO      EOJ
