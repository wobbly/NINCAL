...
PC EQU 0
C EQU 0
Release   Init      "1.32"    RVW	rewrote entire program so is legible.  added verify function which checks if the file exists before writing to log file.  output to user now explains better what is going on.
...
.//		Start over.  Skip those.
		...
		GOTO Main IF (NFTPPROTOCOL = "FTPS_IMPLICIT")	.//Skip targetfiles ftp, I think we don't use them anymore
		GOTO Main IF (NFTPCOMP = "009428")	
		CALL InitMySiteVars
		CALL OpenSite
		CALL Transfer
		CALL DisconnectSite
