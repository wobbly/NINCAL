...
.													  append  " /c copy C:\WORK\hdrfile.prn /b + C:\WORK\faxfile.prn /b \\nins1\d\data\fax\fax",taskname *updated to NTS0\d\data\fax
													  append  " /c copy C:\WORK\hdrfile.prn /b + C:\WORK\faxfile.prn /b \\nts0\d\data\fax\fax",taskname
...
.
	...
	//Only send if attachment is available
		...
		clear	SmtpLogFile
	   	move	"1",SmtpProgress                                    Enable progress bars
		call	SmtpSend   ( 'Send' is in Smtp.Pri which is included in	TestSmtp.Dbs )
.Send them to myself for a little bit for testing
		move	HoldBrkEmail,SmtpTextMessage(1)
		move	"ListManagementPendingOrders@nincal.com",SmtpDestinations(1,1)
		move    "ListManagementPendingOrders@nincal.com",SmtpDestinations(1,2)
		clear	SmtpLogFile
		call	SmtpSend   ( 'Send' is in Smtp.Pri which is included in	TestSmtp.Dbs )
	else
	...
	DISPLAY *P10:17,*EL,"Mail Sent","c:\work\pdf\",str55,".Pdf",*w2
