...
OUTKEY   DIM        78         3-76      .osls,mcomp,oodtem,oodtey
outstr   dim        2
.End Patch #1.2 - Increased Output file and key size cause MCOMP was increased
NUMNEW   FORM       6         74-82
QTYNEW   FORM       9         83-91
...
.         MOVE      "305233" TO NORDFLD            change every jan. previous yr.
         MOVE      "440000" TO NORDFLD            change every jan. previous yr.
          CLOCK     DATE TO DATE
         IFNZ      PC
...
.
         ...
         ENDIF
         CLEAR     OUTKEY
.         PACK      OUTKEY FROM OSLS,MCOMP,OODTEM,"05"
         PACK      OUTKEY FROM mnum,MCOMP
         reset     outkey
...
**********************************************************************
         GOTO      LOOP
WRITE    PACK      OUTKEY FROM mnum,MCOMP       .RESTORE KEY
         reset     outkey
         PACK      outstr FROM OSALES10,OSALES
         MOVE      C0 TO NUMNEW
         MOVE      C0 TO QTYNEW
...
.Printing-Open
        ...
        if (bosflag = c2)
        	PRTOPEN prfile,"\\NTS0\Laser8",str25,noprint,spoolfile=str40
;        	PRTOPEN prfile,"\\NTS0\Laser6",str25,noprint,spoolfile=str40
        else
        	PRTOPEN prfile,"Laser8","List Mgmt Ref",noprint,spoolfile=str40
