...
.END PATCH 1.22 ADDED LOGIC
        ...
        include nusedd.inc
        include winapi.inc
RELEASE   INIT      "1.4"       DLH    .change exclusive option to allow secondary mailer info
Reldate   Init      "2013 May 8"     
...
.                    pack      taskname,"Suppression Reports for Exclusive Lists and List Date Selects do not require a Secondary Mailer!",NewLine,"All Secondary Mailer information will be disregarded."
                    pack      taskname,"Suppression Reports for Exclusive Lists and List Date Selects do not require a Secondary Mailer!",NewLine
.end patch 1.4
                    alert     note,taskname,result
        endif
.END PATCH 1.24 REPLACED LOGIC
...
.          else      .Exclusive/Datacard Date Select
        ...
        SuppressListViewSecondary.GetNextItem giving result using C0,N9
        if (result <> SEQ AND RptFlag = C1)        .we want exclusives  & Suppression
          move      c2,RptFlag
                  move    "00000000",FromDate1                             .usage date
                  getitem SuppressEditFromDate1,0,str10
                  call    Trim using str10
                  count   N9,str10
                  if (N9 <> 0 & N9 <> 8 & N9 <> 10)
...
.
          ...
          Else
                  move    "00000000",FromDate3                           .datacard date
                  getitem SuppressEditFromDate3,0,str10
                  call    Trim using str10
                  count   N9,str10
                  if (N9 <> 0 & N9 <> 8 & N9 <> 10)
...
.
                  ...
                  pack    ToDate3,CC,YY,MM,DD
                    ...
                    getitem SuppressComboDate3,0,DateParam3
.note before patch 1.4 logic was ignoring these dates
                  move    "00000000",FromDate1                             .usage date
                  getitem SuppressEditFromDate1,0,str10
                  call    Trim using str10
                  count   N9,str10
...
.end of new usage date suppress
...
.END PATCH 1.23 REPLACED LOGIC
                                ...
                                if not over
.begin patch 1.4                                
                                    if (rptflag = c2 or (ELSTCDE = "C" or ELSTCDE = "P"))
.end patch 1.4                                
...
.end Patch 1.37
                                                  ...
                                                  if (WithdrawnFlag = C1 OR (STATUS <> "W" AND STATUS <> "T"))
                                                        ...
                                                        move    "SuppressLoad2-NORDAIM",Location
                                                        pack    KeyLocation,"Key: ",NORDFLD1,NORDFLD2
.begin patch 1.4 add code to support order date selects with exclusive only pick
