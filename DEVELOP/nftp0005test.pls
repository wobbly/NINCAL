...
.//IMPORTANT!!!
.//MMI has been moved to the end of the database because connecting to it before doing uploads breaks the Verify function for uploads.
.//You MUST destroy the object in between connections or some junk gets in there and functions don't work.
.//IMPORTANT!!!
...
Release   Init      "2.00"    RVW           rewrote entire program so is legible.  added Verify function which checks if the file exists before writing to log file. output to user now explains better what is going on.
...
MySite     automation      class="CuteFTPPro.TEConnection"
        create  MySite                                            .//Create a new object for each connection, close at the end or the thing has issues
    
.//Open the database that stores the information for the sites 
.//This database is maintained and updated via Program 34
...
Main
           loop
                      destroy MySite                                                    .//destroy in between sites                 
                     Pause "2"
.                      Pause "10"
                      create  MySite                                                    .//Create a new object for each connection, close at the end or the thing has issues
                      ...
                      Read NFTPFILE,seq;NFTPVARS
                                 Until Over
.//                   If the data we pull from the database if for an SMTP server
.//                   Start over.  Skip those.
                      SQUEEZE NFTPPROTOCOL,NFTPPROTOCOL
                      GOTO Main IF (NFTPPROTOCOL = "SMTP")                   .//Skip e-mail protocol                      
                      GOTO Main If (NFTPCOMP != "009406" and NFTPCOMPID != "003")                   .//Skip all but whatever we're testing
                      CALL InitMySiteVars
                      CALL OpenSite
                      CALL Transfer
                      CALL DisconnectSite
...
.//Load the variables into the MySite class
           ...
           MOVE "BINARY",TRANSFERTYPE  if (NFTPCOMP != "009406")
           SQUEEZE TRANSFERTYPE,TRANSFERTYPE
           

           
.           MOVE "PORT",DATACHANNEL 
           
.           Setprop MySite,*Datachannel=DATACHANNEL     
        ...
        If (N9 = c0)
           ...
           Else
                      ...
                      Loop      
                        ...
                        if equal
                        	call checklog                                
                        	CALL GetFile if (check == 2)                                 
                                Call Verify
                        endif
                        reset taskname1
.end 2016 March 10 these appear to be backwards, the reset changes the flags
                      repeat
           Endif
           return     
...
.\\If everything's peachy then we'll write it in the log so we don't try and get that file again.                                              
		...
		If (check == c3)
			Display *P10:19,"Download of ",taskname1
			Display *P10:20,"Skipped because in log file                            "  
			Display *P10:15,"                                                                         "
			Display *P10:17,"                                                                         "
			Display *P10:19,"                                                                         "
			Display *P10:20,"                                                                         "
...
PostFile
           Trap    TrapOLEError giving error if Object
                      If (NFTP2RemoteDir = "/")
                                 Pack CuteRemoteString,"/" .//"Fix color coding in Textedit
                      Else
                                 Pack CuteRemoteString,NFTP2RemoteDir .//"Fix color coding in Textedit
                      Endif
                      ...
                      Display *P10:17,CuteRemoteString
                      
                      
                      Setprop MySite,*Remotefolder=CuteRemoteString
...
CheckLog
	   ...
	   reset taskname1
           ...
           loop
                      ...
                      If Over
                                 move       c2,check                                .//If you didn't find it and reached the end of the database go download the file
                                 return
                      Endif
...
ErrorRename
           ...
           Clear     MailBody
           Append    "Could Not Rename File.  Status is unknown.  This email is sent by program: NFTP0005",Mailbody
