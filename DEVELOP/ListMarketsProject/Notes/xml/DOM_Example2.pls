...
.
	...
	move	"f:\library\develop\xml\DOM_Example2.XML",fileid
.
	formload xmlfm
...
dispnode
	...
	//Let us search for Attributes
	if (parname = "LRNumber")
		getprop	parent,*attributes=attributescol
		getprop	attributescol,*length=result
		move	"0",result3
		loop
			until (result3 >= result)
			getprop attributescol.item(result3),*name=str25
			DOM_ListView.SetItemText using result2,str25,4
			getprop attributescol.item(result3),*text=str25
			DOM_ListView.SetItemText using result2,str25,5
			add	"1",result3
		repeat
	endif

	//Following is just for ease in seeing new node
	if (parname = "NewElement")
		move	"0xFFFF00",colordim                         .Yellow
	else
		move	"0xFFFFFF",colordim                         .White
	endif
