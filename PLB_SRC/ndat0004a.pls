...
.                   *p15:17,"(9)RH Flat File",*rv,*uc,STR1,*lc;
                   *p15:13,"(4)Excel File":
.patch7.18
                   *p15:14,"(5)RH w/ Rev Date",*rv,*uc,STR2,*lc;
.end patch 7.42
.patch7.18
         else
.patch7.18
...
.begin patch 7.0
         MOVE      c0 TO SORT
         MOVE      STR1 TO SORT
         compare   c4 to repflag                     .= fax card
         goto      copies if not equal
.get phone number and name.
whofax   move     "B" to str1
         move     no to faxtele
         keyin     *p1:19,*el,"Fax to (",*cyan,*dion,"B",*white,*hoff:
                   ")roker/Consultant  or (",*cyan,"M",*white,")ailer ":
                   *rv,*t30,*uc,str1,*lc
         move      c0 to telflag
         clear     faxtele
         rep       "b1m2B1M2" in str1
         move       str1 to telflag
         branch     telflag to bhelp,mhelp
         goto       whofax                      .invalid choice. 
bhelp
         KEYIN     *P20:23,"Enter the Number you want (hit enter ":
                   "if finished): ","____",*P70:23:
                        *ZF,*JR,BRKNUM;
         cmatch    b1 to brknum
         goto      whofax if eos
           PACK      NBRKFLD FROM BRKNUM,z3
         SCAN      "?",NBRKFLD
         IF        EQUAL
         clear     brfax
         call      brkhelp
         ...
         move      brfax to faxtele
         move      brcomp to faxname
         DISPLAY    *ef,*P20:23,*EL,"##",brknUM,"  ",brcOMP;
           KEYIN     *P66:23,"OK?",*T254,*uc,STR1,*lc;
           CMATCH    yes TO STR1
           GOTO      testtele IF EQUAL
         goto      bhelp
.        
mhelp
         KEYIN     *P20:23,"Enter the Number you want (hit enter ":
         ...
         goto      whofax if eos
         CLEAR     MKEY
         PACK      MKEY FROM MNUM,z3
         SCAN      "?",mkey
         IF        EQUAL
         call      mlrhelp
         move      mfax to faxtele
         move      mcomp to faxname
         goto      testtele
         endif
         rep       zfill in mkey
           MOVE      C1 TO Nmlrpath
         ...
         move      mfax to faxtele
         move      mcomp to faxname
         DISPLAY    *ef,*P20:23,*EL,"##",mnUM,"  ",mcOMP;
           KEYIN     *P66:23,"OK?",*T254,*uc,STR1,*lc;
           CMATCH    yes TO STR1
           GOTO      testtele IF EQUAL
.        goto      mhelp
testtele type      faxtele
         if        not equal
         display   *p1:24,*el,*b,"Sorry ",faxname,"  does not have fax number!!",*w3
         goto       keytele
.begin patch 6.2
         endif
.end patch 6.2
         unpack    faxtele into arcd,exch,tele
         match     "000" to exch
.begin patch 6.2
.         goto      keytele if equal
         goto      badtele if equal
.end patch 6.2
         cmatch    b1 to exch
.begin patch 6.2
..         goto      keytele if eos
...
.begin patch 6.2
badtele
         display   *p1:24,*el,*b,"Sorry ",faxname,"  has an invalid fax number!!",*w3
.end patch 6.2
keytele  move      yes to str1
         keyin     *p1:24,*el,"Enter the fax number: (___)___-___",*p24:24,*+,arcd:
                   *p28:24,exch,*p32:24,tele,*p69:24,"OK?",*rv,*uc,str1,*lc,*it
         cmatch    yes to str1
         goto      keytele if not equal
         pack      faxtele from arcd,exch,tele
         scan      star in faxtele
         goto      sortopt if equal
         reset     faxtele
         type      faxtele
         if        not equal
         display   *p1:24,*el,*b,"Sorry invalid fax number!!",*w3
         goto      keytele
.         endif
         branch    telflag of upbrk,upmlr
         goto      sortopt
upbrk
         move     c0 to brkcnt
         move     c0 to n3
         move     c1 to nbrkpath
         move     faxtele to brfax
                              
. WRTUPD1 - PARTIAL UPDATE OF OTHER CONTACTS, CHANGING fax iNFO ONLY..
WRTUPD1
                                        GOTO      keysub
.Patch7.20
.;                            CLEAR     nbrkfld
...
.;         call      nbrkkey
.;         GOTO      keysub IF OVER
.;         DISPLAY   *P60:24,"Updating",brknum,SLASH,brkcnt;
.;         filepi    1;nbrkfile
.Tab pushed over to reflect increase in NINBRK.DAT
.         UPDATAB   NbrkFILE;*140,brFAX
.         UPDATAB   NbrkFILE;*182,brFAX
.;         UPDATAB   NbrkFILE;*222,brFAX
...
upmlr
         ...
         move      z3 to n3
         move      c1 to nmlrpath
         move      faxtele to mfax
. WRTUPD2 - PARTIAL UPDATE OF OTHER CONTACTS, cHANGING fax number INFO ONLY..
WRTUPD2  
.Patch7.20
.;                            CLEAR     MKEY
.;         PACK      MKEY FROM MNUM,MCONTCT
.;         move      c1 to nmlrlock                  .turn on locking
.;         REP       ZFILL,MKEY
.;         call      nmlrkey
.;         GOTO      keysub IF OVER
.;         DISPLAY   *P60:24,"Updating",MNUM,SLASH,MCONTCT;
.;         filepi    1;nmlrfile
.Start Patch #6.1 - amend UPDATAB to reflect NINMLR expansion
.         UPDATAB   NMLRFILE;*158,MFAX
.;         UPDATAB   NMLRFILE;*238,MFAX
.End Patch #6.1 - amend UPDATAB to reflect NINMLR expansion
GOON2
.;                            ADD       C1,n3
.;         MOVE      n3,MCONTCT
.;         RESET     MCONTCT TO 3
.;         RESET     MCONTCT
.;         REP       ZFILL,MCONTCT
.;         move      c3 to nmlrlock          .turn off locking
.;         GOTO      WRTUPD2
         endif
.Patch7.20
..................
keysub   MOVE      "DATACARDS" TO SUBJECT
         keyin     *p1:04,*LC,*it,*ef,"To: ",*dv,faxname,*p5:04,*rv,*edit,*t30,faxname:
                   *DV,b3,*dv,faxtele     
         keyin     *p1:05,*el,"Subject  :",*DV,SUBJECT,*P9:05,*rv,*edit,*t30,subject
         move      c0 to blincnt
         clear     fline1
         clear     fline2
         clear     fline3
         ...
         clear     fline20
         clear     fline99
fline1   move      fline1 to fline99
         keyin     *p1:06,*el,"Note: ",*dv,fline1,*p7:06,*rv,*edit,*t30,fline1
         cmatch    b1 to fline1
         if        eos
         add       c1 to blincnt
         move      b2 to fline1
         endif
fline2   move     fline2 to fline99  
         keyin     *p7:07,*el,*dv,fline2,*p7:7,*rv,*edit,fline2
         cmatch    b1 to fline2
         if        eos
         add       c1 to blincnt
         compare   c2 to blincnt
         goto      copies if equal
         move      b2 to fline2
         goto      fline3
         endif
         move     c0 to blincnt
         cmatch   "<" to fline2
         if       equal
         move     fline99 to fline2
         display  *p7:7,fline2
         goto     fline1 
         endif
fline3   move      fline3 to fline99
         keyin     *p7:08,*el,*dv,fline3,*p7:8,*rv,*edit,fline3
         cmatch    b1 to fline3
.         goto     copies if eos
         if         eos
         add       c1 to blincnt
         ...
         cmatch   "<" to fline3
         if       equal
         move     fline99 to fline3
         display  *p7:8,fline3
         goto     fline2 
         endif
fline4   move      fline4 to fline99
         keyin     *p7:09,*el,*dv,fline4,*p7:9,*rv,*edit,fline4
         cmatch    b1 to fline4
.         goto     copies if eos
.         goto     copies if eos
         if         eos
         ...
         cmatch   "<" to fline4
         if       equal
         move     fline99 to fline4
         display  *p7:9,fline4
         goto     fline3 
         endif
fline5   move      fline5 to fline99
         keyin     *p7:10,*el,*dv,fline5,*p7:10,*rv,*edit,fline5
         cmatch    b1 to fline5
.         goto     copies if eos
.         goto     copies if eos
         if         eos
         ...
         cmatch   "<" to fline5
         if       equal
         move     fline99 to fline5
         display  *p7:10,fline5
         goto     fline4 
         endif
fline6   move      fline6 to fline99
         keyin     *p7:11,*el,*dv,fline6,*p7:11,*rv,*edit,fline6
         cmatch    b1 to fline6
.         goto     copies if eos
.         goto     copies if eos
         if         eos
         ...
         cmatch   "<" to fline6
         if       equal
         move     fline99 to fline6
         display  *p7:11,fline6
         goto     fline5 
         endif
fline7   move      fline7 to fline99
         keyin     *p7:12,*el,*dv,fline7,*p7:12,*rv,*edit,fline7
         cmatch    b1 to fline7
.         goto     copies if eos
.         goto     copies if eos
         if         eos
         ...
         cmatch   "<" to fline7
         if       equal
         move     fline99 to fline7
         display  *p7:12,fline7
         goto     fline6 
         endif
fline8   move      fline8 to fline99
         keyin     *p7:13,*el,*dv,fline8,*p7:13,*rv,*edit,fline8
         cmatch    b1 to fline8
.         goto     copies if eos
         if         eos
         add       c1 to blincnt
         ...
         cmatch   "<" to fline8
         if       equal
         move     fline99 to fline8
         display  *p7:13,fline8
         goto     fline7 
         endif
fline9   move      fline9 to fline99
         keyin     *p7:14,*el,*dv,fline9,*p7:14,*rv,*edit,fline9
         cmatch    b1 to fline9
.         goto     copies if eos
         if         eos
         add       c1 to blincnt
         ...
         cmatch   "<" to fline9
         if       equal
         move     fline99 to fline9
         display  *p7:14,fline9
         goto     fline8
         endif
fline10  move      fline10 to fline99
         keyin     *p7:15,*el,*dv,fline10,*p7:15,*rv,*edit,fline10
         cmatch    b1 to fline10
.         goto     copies if eos
         if         eos
         add       c1 to blincnt
         compare   c2 to blincnt
         ...
         cmatch   "<" to fline10
         if       equal
         move     fline99 to fline10
         display  *p7:15,fline10
         goto     fline8 
         endif
fline11   move      fline11 to fline99
         keyin     *p7:16,*el,*dv,fline11,*p7:16,*rv,*edit,fline11
         cmatch    b1 to fline11
.         goto     copies if eos
         if         eos
         add       c1 to blincnt
         compare   c2 to blincnt
         ...
         cmatch   "<" to fline11
         if       equal
         move     fline99 to fline11
         display  *p7:16,fline11
         goto     fline10 
         endif
         goto     fline10 if equal
fline12  move      fline12 to fline99
         keyin     *p7:17,*el,*dv,fline12,*p7:17,*rv,*edit,fline12
         cmatch    b1 to fline12
.         goto     copies if eos
         if         eos
         add       c1 to blincnt
         compare   c2 to blincnt
         ...
         cmatch   "<" to fline12
         if       equal
         move     fline99 to fline12
         display  *p7:17,fline12
         goto     fline11 
         endif
fline13  move      fline13 to fline99
         keyin     *p7:18,*el,*dv,fline13,*p7:18,*rv,*edit,fline13
         cmatch    b1 to fline13
.         goto     copies if eos
         if         eos
         add       c1 to blincnt
         compare   c2 to blincnt
         ...
         cmatch   "<" to fline13
         if       equal
         move     fline99 to fline13
         display  *p7:18,fline13
         goto     fline12 
         endif
fline14  move      fline14 to fline99
         keyin     *p7:19,*el,*dv,fline14,*p7:19,*rv,*edit,fline14
         cmatch    b1 to fline14
.         goto     copies if eos
         if         eos
         add       c1 to blincnt
         compare   c2 to blincnt
         ...
         cmatch   "<" to fline14
         if       equal
         move     fline99 to fline14
         display  *p7:19,fline14
         goto     fline13 
         endif
fline15  move      fline15 to fline99
         keyin     *p7:20,*el,*dv,fline15,*p7:20,*rv,*edit,fline15
         cmatch    b1 to fline15
.         goto     copies if eos
         if         eos
         add       c1 to blincnt
         compare   c2 to blincnt
         ...
         cmatch   "<" to fline15
         if       equal
         move     fline99 to fline15
         display  *p7:20,fline15
         goto     fline14 
         endif
fline16  move      fline16 to fline99
         keyin     *p7:21,*el,*dv,fline16,*p7:21,*rv,*edit,fline16
         cmatch    b1 to fline16
.         goto     copies if eos
         if         eos
         add       c1 to blincnt
         compare   c2 to blincnt
         ...
         cmatch   "<" to fline16
         if       equal
         move     fline99 to fline16
         display  *p7:21,fline16
         goto     fline15 
         endif
fline17   move      fline17 to fline99
         keyin     *p7:22,*el,*dv,fline17,*p7:22,*rv,*edit,fline17
         cmatch    b1 to fline17
         goto     copies if eos
         cmatch   "<" to fline17
         if       equal
         move     fline99 to fline17
         display  *p7:22,fline17
         goto     fline16 
         endif
.
...
.
RETRIEVE DISPLAY   *P1:24,*EF,*P7:24,"Records searched,       ":
...
.         branch    repflag of card,rh,bstock,faxcard,lcard,cardview,attcard,nother,RHFLAT
         branch    repflag of card,rh,bstock,faxcard,lcard,cardview,attcard,nother,RHFLAT,RHREV
...
ATTCARD
         clear     taskname
.begin patch 7.32
.           append    "c:\progra~1\lanbatch\batch -X -SA -Q\\nts0\c\lanbat~2 f:\apps\winbatch\butil job=faxcard2 INfile=",TASKNAME
.                    IF  (Bit64Flag = No)    .if not 64 bit we can submit to batch else run locally
.                    append    "\\Nins1\Lanbatch\batch32 -X -SA -Q\\nins1\ServerA \\Nins1\Winbatch\butil job=faxcard2 INfile=",TASKNAME
.                    else
                    append    "\\Nins1\Winbatch\butil job=faxcard2 INfile=",TASKNAME
.                    endif
           APPEND    SHORTNME TO TASKNAME
.end patch 7.32
.           APPEND    " F=default C=",TASKNAME
...
.START PATCH 6.6 REPLACED LOGIC
.             match     "415" to str3            . local?
             match     "510" to str3            . local?
.END PATCH 6.6 REPLACED LOGIC
              IF         EQUAL                    
         ...
         clock     date to today
.begin patch 6.5
.         print     "^[D",longdist,STR1,faxtele,"^[N",faxname:
...
.                   hpltrhd
.START PATCH 7.21 REPLACED LOGIC
.         print     "^[D",longdist,STR1,faxtele,"^[N",faxname:
.                   "^[T",today,b1,str5,"^[S",NUSEUSER,"^]":
.                   *n,032,hpreset:
.                   hpttray:
...
.;..............................................................................
.;           PRINT     *N,*N,*N,*n:
.;                   *n,*n, *1,hp14pt,HPBON,hpunon,"                         ":
.;                    "FACSIMILE TRANSMITTAL COVER SHEET                 ":
.;                    "                           ",HPUNOFF,HPBOFF:
.;                    *N,*n,*5,"Please deliver the following pages to:"
.           PRINT     *N,*N,*N,HPTMSRMN,hpfixed,*n:
.                   *n,*n,*n,*n,*1,hp14pt,HPBON,hpunon,"                         ":
.                    "FACSIMILE TRANSMITTAL COVER SHEET                 ":
.                    "                           ",HPUNOFF,HPBOFF:
.                    *N,*n,*5,"Please deliver the following pages to:"
.;end patch 6.5
.         PRINT      *N,*N,*5,"Name    : ",hpunon,faxname,hpunoff
.         PRINT      *N,*5,"From    : ",nuseuser
.         PRINT      *N,*5,"Subject : Datacards"
.         print      *n,*n,fline1:
.                    *n,*5,fline2:
.                    *n,*5,fline3:
.                    *n,*5,fline4:
...
.
          ...
          move      "1500",row
          prtpage   prfile;*pcolumn3:row,*font=font1,"FACSIMILE TRANSMITTAL COVER SHEET"
          add       eightlpi,row
          add       eightlpi,row
          prtpage   prfile;*pcolumn:row,*LINE=7500:row
          add       eightlpi,row
          add       eightlpi,row
          prtpage   prfile;*pcolumn1:row,*font=font2,"Please deliver the following pages to:"
          add       eightlpi,row
          add       eightlpi,row
          add       eightlpi,row
          prtpage   prfile;*pcolumn1:row,"Name"
          prtpage   prfile;*pcolumn2:row,":"
          prtpage   prfile;*pcolumn3:row,*ULON,faxname,*ULOff
          add       eightlpi,row
          add       eightlpi,row
          add       eightlpi,row
          prtpage   prfile;*pcolumn1:row,"From"
          prtpage   prfile;*pcolumn2:row,":"
          prtpage   prfile;*pcolumn3:row,nuseuser
          add       eightlpi,row
          add       eightlpi,row
          add       eightlpi,row
          prtpage   prfile;*pcolumn1:row,"Subject"
          prtpage   prfile;*pcolumn2:row,":"
          prtpage   prfile;*pcolumn3:row,"Datacards"
          add       eightlpi,row
          add       eightlpi,row
          add       eightlpi,row
          add       eightlpi,row
          add       eightlpi,row
          prtpage   prfile;*pcolumn:row,fline1
          add       eightlpi,row
          add       eightlpi,row
          prtpage   prfile;*pcolumn:row,fline2
          add       eightlpi,row
          add       eightlpi,row
          prtpage   prfile;*pcolumn:row,fline3
          add       eightlpi,row
          add       eightlpi,row
          prtpage   prfile;*pcolumn:row,fline4
          add       eightlpi,row
          add       eightlpi,row
          prtpage   prfile;*pcolumn:row,fline5
          add       eightlpi,row
          add       eightlpi,row
          prtpage   prfile;*pcolumn:row,fline6
          add       eightlpi,row
          add       eightlpi,row
          prtpage   prfile;*pcolumn:row,fline7
          add       eightlpi,row
          add       eightlpi,row
          prtpage   prfile;*pcolumn:row,fline8
          add       eightlpi,row
          add       eightlpi,row
          prtpage   prfile;*pcolumn:row,fline9
          add       eightlpi,row
          add       eightlpi,row
          prtpage   prfile;*pcolumn:row,fline10
          add       eightlpi,row
          add       eightlpi,row
          prtpage   prfile;*pcolumn:row,fline11
          add       eightlpi,row
          add       eightlpi,row
          prtpage   prfile;*pcolumn:row,fline12
          add       eightlpi,row
          add       eightlpi,row
          prtpage   prfile;*pcolumn:row,fline13
          add       eightlpi,row
          add       eightlpi,row
          prtpage   prfile;*pcolumn:row,fline14
          add       eightlpi,row
          add       eightlpi,row
          prtpage   prfile;*pcolumn:row,fline15
          add       eightlpi,row
          add       eightlpi,row
          prtpage   prfile;*pcolumn:row,fline16
          add       eightlpi,row
          add       eightlpi,row
          prtpage   prfile;*pcolumn:row,fline17
          prtclose prfile
.
tester
          clear   taskname
          ...
          append  "copy ",taskname
          append  recname,taskname
          append  " /b + c:\work\faxfile.prn /b c:\work\faxfile2.prn",taskname
          reset   taskname
          execute taskname
.
          copyfile "c:\work\faxfile2.prn",recname
          erase     "c:\work\faxfile2.prn"
.END PATCH 7.21 REPLACED LOGIC
         release
         goto      weoffile
.old code
         MOVE      c1 to n2
         WRITE     FORMFILE,seq;"SMF-70"
         WRITE     FORMFILE,seq;"o-nw-formtype: n,w, #"Datacards#""
         WRITE     FORMFILE,seq;"TO: Fax @ FACSys ( ) {FAX: ":
                   LONGDIST,B1,*LL,faxtele;
         WRITE     FORMFILE,seq;"; TO: ",faxname,"}"
         WRITE     FORMFILE,seq;"From: ",nuseuser
         WRITE     FORMFILE,seq;"Subject: ",subject
         WRITE     FORMFILE,seq;"ATTACHMENT: ",*LL,shortnme,".fax"
         WRITE     FORMFILE,seq;"ATTACHMENT-NAME: ",*LL,shortnme,".fax"
         cmatch    b1 to fline1
         if        eos
         add       c1 to blincnt
         endif
.         goto      weoffile if eos
.         goto      weoffile if equal
         write     formfile,seq;b1
         write     formfile,seq;fline1
         cmatch    b1 to fline2
.         goto      weoffile if eos
.         goto      weoffile if equal
         if        eos
...
.         goto      weoffile if equal
         ...
         write     formfile,seq;fline17
WEOFFILE                               
.         WEOF      FORMFILE,SEQ,chop
.         CLOSE     FORMFILE            
         trap      drivea if io
         prepare   formfile,"c:\work\dsprog.dat"
         trapclr   io
         goto      wrtdsprg
drivea   
         prepare   formfile,"a:\dsprog.dat"
wrtdsprg write     formfile,seq;"$PROGRAM=NDAT0002"
         write     formfile,seq;"$COMPANY=",Company
         write     formfile,seq;"$FUNC=2"
         write     formfile,seq;"$INITS=",copy
         write     formfile,seq;"$USER=",user
         write     formfile,seq;"$PRTNAME=",shortnme
         write     formfile,seq;"$INPNAME=",shortnme,".srt"
         write     formfile,seq;"$comment=FAX,LASER"
         write     formfile,seq;"$EXIT=1"
         WEOF      FORMFILE,SEQ
         CLOSE     FORMFILE
         clear     taskname
.         APPEND    "f:\netutils\SORT g:\DATA\" TO TASKNAME
.         APPEND    SHORTNME TO TASKNAME
.         APPEND    ".DAT G:\DATA\" TO TASKNAME
.         APPEND    SHORTNME TO TASKNAME
.         APPEND    ".SRT /SORT(72,26,c,a) w(c:) f(pcb)" TO TASKNAME        
.         RESET     TASKNAME
.         EXECUTE   TASKNAME
.         chain     dsinit
.         append    "y:command.com /C datasend ", TASKNAME 
.         append    "c:\command.com /C datasend.bat ", TASKNAME 
.  dave goes bad
.START PATCH 7.02 REPLACED LOGIC
.         append     "f:\netutils\SORT g:\data\",taskname
.         append     shortnme to taskname
.         append     ".dat g:\data\" to taskname
         append     NTWKPATH2,taskname
         append     "SORT ",taskname
...
.END PATCH 7.02 REPLACED LOGIC
         append     shortnme to taskname
.         append     ".srt /sort(72,26,c,a) w(c:) ",taskname
         CLOCK      VERSION TO STR25
         SCAN       "PLBWIN" IN STR25
         IF          EQUAL
         append     ".srt /sort(72,26,c,a) w(c:) f(pcb) ",taskname
         ELSE
         append     ".srt /sort(72,26,c,a) w(c:) f(pcb) ",taskname
         ENDIF
         reset      taskname
...
.         append     "\\nins1\e\apps\plb\code\plb386.exe \\nins1\e\apps\plb\code\DSINIT",taskname
         append     "\\nins1\e\apps\plb\code\plbcon.exe \\nins1\e\apps\plb\code\DSINIT",taskname
         ENDIF
         reset      taskname
         execute    taskname 
         clear      taskname
