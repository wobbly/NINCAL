...
release   init    "10.52"     DLH  .rewrite sample code
REldate   Init      "2016 April 13"      
...
.end patch 10.4
.begin patch 10.51
.begin patch 10.52      Do this if hot or regular run
.                      if         (hotflag = 2)
.end patch 10.52      Do this if hot or regular run
          ...
          if        (formflag = 2)
           ...
           pack      KeyLocation,"Key: ",NSMPFLD
                                 if         (oscode = "1")                   .sample enclosed
                                            ...
                                            call   trim using nsmpdes1
                                            clear   taskname
                                            pack    Taskname from "<a href=#"http://www.nincal.com/data/samples/S",NSmpfld,".pdf","#">",Mcomp,"-",nsmpdes1,"</a>"
.add code to check for duplicates ?  ie count smpmailbody if zero proceed, if not reset smpmailbody, scan taskname in smpmailbody if equal append smpmailbody to 
.smpmailbody to set the string up again, else append taskname to smpmailbody --- check the logic before implementing
                                                       if         (SampleList = "")
                                                       append  Taskname,SmpMailBody
                                                       append     "<br>",SmpMailBody
                                                       add        c1,ownscnt
                                                       pack       SampleList from NSmpFld
                                                       else
.scan did we already add this link?
                                                       scan       NSmpFld in Samplelist
                                                                  if         equal      
.                                                                                       .Do Nada
                                                                  reset      samplelist
                                                                  else                   . Do the usual and add to the list
                                                                  append     Taskname,SmpMailBody
                                                                  append     "<br>",SmpMailBody
                                                                  reset      samplelist
                                                                  pack       SampleList from SampleList,comma,NSmpFld         
                                                                  add        c1,ownscnt
                                                                  endif
                                                       endif           
                                            endif
