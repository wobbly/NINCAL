...
.testing small form
release  init      "1.60"          DLH     .$ variance needs to include adjustments
reldate   Init      "2015 September xx"
.release  init      "1.54"          DLH     auto refresh of Managed list file
.reldate   Init      "2014 April 9"
.release  init      "1.53"          DLH     added Flags= to prtopen with PDF:
...
.RELEASE  INIT      "1.52"        DLH  replace laser3 with Laser4
.REldate   Init      "2013 December 12"
.RELEASE  INIT      "1.51"        DLH  f10 dumps all managed files into the datalist
.REldate   Init      "2013 June 7"
.RELEASE  INIT      "1.50"        DLH  Sunbelt PDF
.REldate   Init      "2013 April 24"
.RELEASE  INIT      "1.44"        DLH  Add code so mailsend of PDF works under plbclient
.REldate   Init      "11 March 2010"
.RELEASE  INIT      "1.43"        DLH  add filecheck
.REldate   Init      "Feb 2010"
.RELEASE  INIT      "1.42"        JD  reset Mailto/mailbody var for correct sendmail
.REldate   Init      "09 July 2008"
.RELEASE  INIT      "1.41"       DLH correct copyright
.REldate  Init      "06 May 2008"
.RELEASE  INIT      "1.40"       ASH    20JUN2007 PLI Inclusion
.REldate  Init      "20 June 2007"
.                             .Note plf issues compiling 1.4.0 cleaned up DLH
...
.START PATCH 1.40 ADDED LOGIC
font14    font
          create  font14,"Times New Roman",size=14,bold
font15    font
          create  font15,"Times New Roman",size=7
.END PATCH 1.40 ADDED LOGIC
...
srch      plform  Search
abt       plform  About
x         PLFORM  \\nins1\e\library\plb_src\NVAR0001
.===============================================================================
        winhide
        FORMLOAD x
        formload srch
...
.=========================================================================
          clear n4
          getitem NvarComboBox001,n4,n3
          move n3 to HoldReport
          getitem NvarComboBox002,n4,vardateflag  
          if (holdreport <> c5)
...
.begin patch 1.54
           pack       taskname from NTWKPATH6,"DATAEXCL.EOQ"
           Prepare    Exclude,taskname,exclusive
          move        c4,ndatpath
          
          loop
          call        ndatks
          until over
           if         ((elstcde = "C" or Elstcde = "P") & status <> "W")         .
          write       exclude,seq;lstnum,b5,Olstname
           endif
          repeat
          weof        exclude,seq
          close       exclude
...
.end patch 1.54
          pack   SortFle,NTWKPATH6,"DATAEXCL.EOQ",comma,prtdir,"DATAEXCL.SRT"
                ...
                goto COPE
                  ...
                  clear citem
                  clear ditem
                  getitem NVAR0001DataList,c1,n4
                  if (n4 = c0)
...
.Get Printer
          ...
          if (PrintArea = "1")
                    move "\\NINS2\laser8" to PrinterName
          elseif (PrintArea = "2")
                    move "\\NINS2\laser4" to PrinterName     
          elseif (PrintArea = "3")
.Begin patch 1.50
.                   move "PDF995" to PrinterName  
                   move "PDF:" to PrinterName  
.end patch 1.50
          elseif (PrintArea = "4")      
                    clear printername
          elseif (PrintArea = "7")      
                    move "\\NWK0100\Kyoceram" to PrinterName     
          endif
          move c1 to nordpath           
********************************************************************
...
Startprint
.begin patch DH test
           if         (result = seq)          .listview was empty
.           goto       Here   Crap not there going to have to trace it thru
           endif
.end patch DH test
........................................................
.Position of Columns
...
.begin patch 1.5
          if        (PrintArea = "3")
          PRTOPEN   prfile,PrinterName,prtfile1,FLAgs=PDF_FLAGS_WIN_ANSI_ENCODING
          else
...
.                    erase     "c:\progra~1\pdf995\flag.dat"  
                    ...
                    move    "Here is your variance report.",MailSubjct
                    Clear     Mailbody
...
.                    pack      MailAttach from "c:\work\pdf\",str55              ."
                    pack      Mailattach from prtfile1
.                    pack      taskname from "!c:\work\pdf\",str55              ."
                    pack      taskname from "!",prtFile1                  ."
                    Pause     C5
...
.DISABLES OBJECTS
          ...
          setprop NvarComboBox001,Enabled=c0
          setprop NvarComboBox002,Enabled=c0
...
.ENABLES OBJECTS
          ...
          setprop NvarComboBox001,Enabled=c1
          setprop NvarComboBox002,Enabled=c1      
...
LoadExcl
        ...
        Move      c4,Ndatpath
        move      "999999",ndatfld
        move    "LoadEXCL-NDATkey",Location
        call      Ndatkey
        loop
        move    "LoadEXCL-NDATKP",Location
        call NdatKP
        until over
          getitem NVAR0001DataList,c1,n4
          
        rep         zfill,lstnum  
          setitem  NVAR0001EditList,0,lstnum
          setitem  NVAR0001StatListName,0,olstname
        insertitem NVAR0001DataList,9999,LSTNUM
        repeat
         getitem NVAR0001DataList,c1,n4
       return
...
DelList
          ...
          deleteitem NVAR0001DataList,n4
        move c0 to Exflg
          return
...
.         "©","1995-2005, Names in the News";
.        if (HOLDEXCL = "P")
.                 prtpage prfile;*pColumn:row,*font=font9,*ALIGNMENT=*Left:
.                   "©","2007, Pacific Lists";
.         else
                  prtpage prfile;*pColumn:row,*font=font9,*ALIGNMENT=*Left:
                    "©","1995-2016, Names in the News";
.         endif
...
.START PATCH 1.40 REPLACED LOGIC
.         prtpage   prfile;*units=*HIENGLISH,*Pictrect=*on,*PICT=200:650:200:1200:NINLogo;        
          if (HOLDEXCL = "P")
                    prtpage   prfile;*p=1030:50,*font=font14,*Alignment=*Center,"Pacific Lists, Inc.":
                              *p=1030:275,*font=font15,"180 Grand Avenue, Suite 1365":
                              *p=1030:375,"Oakland, CA 94612-3716":
                              *p=1030:475,"415-945-9450 ","·"," Fax 415-945-9451":
                              *p=1030:575,"A Division of Names in the News":
                              *Alignment=*Left
          else
                    prtpage   prfile;*units=*HIENGLISH,*Pictrect=*on,*PICT=200:650:200:1200:NINLogo;        
          endif
.END PATCH 1.40 REPLACED LOGIC
.        prtpage prfile;*pColumn:row,*ALIGNMENT=*Left,*font=font8,"Confidential";
...
.                   if        (trapcount > 60)   . 5 min are you kidding me
                    ...
                    Pack       MailSubjct,"Nvar0001 - ",str35,b1,str55
...
.added for plbserver runs does not like "\\NINS2\laser?"         gets s10 if s10 try this
         TRAPCLR   SPOOL
