...
release        init           "1.5"        DLH    .Check for orginal mail date. corrected ninsteb file name this program was using ninsteb|10.10.30.103:502
.                                                 variable was not large enough 
.RelDate        Init           "March 6, 2012"
.release        init           "1.4"        DLH    .sort out order records, use an array while collating data, job now takes about 2 minutes instead of 30+
...
.....................................................
.begin patch 1.4
DTPREcord REcord         (350)                                   ;artificial cap
DTPIDRec  Dim            12             01-12              Broker # & Mailer #
DTP3YR    FOrm           5              13-17              3 year count
DTP3YRDay Form           9              18-26              3 year total days
DTP1YR    FOrm           5              27-31              1 year count
DTP1YRDay Form           9              32-40              1 year total days
               recordend
RecID     Form      3
...
EditTextBoxes   EditText (3)
               ...
               SetProp        Generic001,Title="Neom0036"
               create         Generic001;StatTextBoxes(1)=50:70:10:110,"Status:",""
               create         Generic001;EditTextBoxes(1)=50:70:80:430,MaxChars=50,EditType=2,SelectAll=1,Style=1,Border=1
...
.               Prepare        DTPFIle,"\\nins1\e\data\DaystoPay.dat","\\nins1\e\data\daystopay.isi","12","44",exclusive
               Prepare        DTPFIle,"\\nins1\e\data\text\DaystoPay.dat","\\nins1\e\data\index\daystopay.isi|nins1:502","12","44",exclusive
...
.begin patch xxx
          ...
          rep       zfill in str6
          setitem   EditTextBoxes(1),0,"Sorting to Norddays"
          pack      taskname from "\\nins1\e\data\text\ninord.dat, \\nins1\e\data\norddays.dat /p=#"91>'",str6,"'#""
          sort      taskname,SUNDM="nins1:502"
          setitem   EditTextBoxes(1),0,"Aamdexing Norddays"
          aamdex    "\\nins1\e\data\norddays.dat,\\nins1\e\data\norddays.aam -3-6,16-21,26-37,303-306,22-25,52-63,198-199",SUNDM="nins1:502"
          Open      NORDFLE2,"\\nins1\e\data\norddays|NINS1:502"
         MOVE      C1 TO NORDFLG2
.end patch xxx
          move    "ninsteb|nins1:502" to nstename
Main

          setitem   EditTextBoxes(1),0,"Processing ninsteb"
               call           nsteseq
.BEGIN PATCH 1.4
          If        Over
          Call      WriteDTP
...
.END PATCH 1.2 REPLACED LOGIC
.Begin patch 1.4
          For       recid,"1","550"                    .note if break this we need to increase the record size)
          Clear     Str12
          move      DTPREcord(REcId).DTPIDREC to str12
          if        (str12 = "")         .dont have this record yet
          Break
          Elseif    (dtpfld = str12)    .already processed
          goto      main
          endif
          Repeat
.               read           DTPFIle,DTPFld;;
...
.end Patch 1.4
              pack  str14 from stebrk,slash,stemlr
              setitem   EditTextBoxes(1),0,str14
...
.Begin patch 1.4
          ...
          move      DTPREcord(REcId).DTPIDREC to str12
          if        (str12 = "")         .dont have this record yet
          MOve      DTPFLD,DTPREcord(REcId).DTPIDREC
                    if        (Flag12Mos = yes)
                    add       c1,DTPREcord(REcId).DTP1YR
                    add       Ndays,DTPREcord(REcId).DTP1YRday
                    endif
          add       c1,DTPREcord(REcId).DTP3YR
          add       Ndays,DTPREcord(REcId).DTP3YRday
          Break
          elseif    (DTPFLD = str12)
                    if        (Flag12Mos = yes)
                    add       c1,DTPREcord(REcId).DTP1YR
...
WriteDTP
          For       recid,"1","550"                    .note if break this we need to increase the record size)
          move      DTPREcord(REcId).DTPIDREC to DTPFLD
          if        (DTPFLD = "" or str12 = b1)         .done
          Break
          endif
          unpack    DTPFLD,DTPBRoker,DTPMailer
          Move      DTPREcord(REcId).DTP3YR,DTP3YrCount
          Move      DTPREcord(REcId).DTP3YRDay,DTP3YrDays
          Move      DTPREcord(REcId).DTP1YR,DTP1YrCount
          Move      DTPREcord(REcId).DTP1YRDay,DTP1YrDays
          call      debug
