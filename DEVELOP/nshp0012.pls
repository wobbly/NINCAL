...
.         pack DateString,mm,slash,dd,slash,cc,yy
        ...
        create  Red=255:0:0
         ...
         OPEN      LOGFILE,"Shipfax|10.10.30.103:502"         *TURNED On 12/08/94.
...
.begin patch 1.31
          call      debug
.           GETPROP  books,*xlwindowview=Window
.           GETPROP  Window,*View=View  
.           SETPROP  book,*Type=XLNormalView   
.begin patch 1.31
...
.begin patch 1.31   cellrange).merge generates an O145 error with excel 2013
        setprop ex,*DisplayAlerts=OFALSE
...
.                   setprop sheet.range(CellRange).Select,*Mergecells=OTrue
.                   Select.MergeArea
        setprop ex,*DisplayAlerts=OTRUE
.                  setprop sheet.range("G1:G2"),*Value="Shipping Report",*HorizontalAlignment=AlignCenter                   
.                  setprop sheet.range("G1:G2").Font,*Bold="True"         
.                  setprop sheet.range("G1:G2").Font,*Name="Arial"        
.                  setprop sheet.range("G1:G2").Font,*Size="16"
.                  setprop sheet.range("G1:G2"),*HorizontalAlignment=xlCenter      
.                  setprop sheet.range("G1:G2"),*VerticalAlignment=xlCenter 
.                  setprop sheet.range(Cell),*Value="Shipping Report",*HorizontalAlignment=AlignCenter                   
.                  setprop sheet.range(Cell).Font,*Bold="True"         
.                  setprop sheet.range(Cell).Font,*Name="Arial"        
.                  setprop sheet.range(Cell).Font,*Size="16"
.                  setprop sheet.range(Cell),*HorizontalAlignment=xlCenter      
.                  setprop sheet.range(Cell),*VerticalAlignment=xlCenter 
.end patch 1.31   cellrange).merge generates an O145 error with excel 2013 so easy fix - don' need it on this report so don't use
...
ShipCostClean       
                    ...
                    Goto ShipCostClean2 if (COMPHOLD = "009410" & IDMIflag = yes)    .Frontline
...
.         Move "Dawn.Michaelis@infoUSA.com",Mailto
          Move "Dawn.Michaelis@infoUSA.com",MailCC
