...
.          else
              pack            Taskname,"\\nins1\d\accounting\monday2015.xlsx"       
.              endif
.end patch 3.79
...
. Projection Section Update Yearly!! checking for current year projections
          packkey    nPrjfld with TYPE,SRC,cid,"201501"
                 call    nprjkey
                    If        (PrjAR > 0)                    
                    MOve      No,Newbiz
                 return
...
.needs previous year if $ its not new
                 pack    nrevfld,typer,srcr,cidr,"2014" ......<<<<<<<<Yearly??????????????????????????
                 call    nrevkey
...
.CHANGE ANNUALLY                 
                 ...
                 if      equal
.needs previous year if $ its not new
                 pack    nrevfld from "EB",srcr,cidr,"2014"            ....YEARLY
                 else
                 pack    nrevfld from "RB",srcr,cidr,"2014"
                 endif
                 call    nrevkey
                 return
.end patch 3.95          
