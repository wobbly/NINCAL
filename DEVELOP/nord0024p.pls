...
;.           RECORDS FOR TRANSMISSION TO PIDI XML FORMAT.
;. 
PC       EQU       0
         INCLUDE    COMMON.inc
         INCLUDE   CONS.inc
...
; FILES.
; ......
PIDIORD   FILE               .TARG ORDER INPUT FILE.
...
; ...........
         ...
         MOVE      "PIDI SPOOL PROGRAM" TO STITLE
         CALL       PAINT
;START PATCH 2.56 ADDED LOGIC
...
;         GOTO      NOTDMC IF NOT EQUAL
	...
	if (OWNCTN <> "")
		...
		call	NFULKEY
		match "0007" to nfulnum
		goto  goodone if equal
		else
		clear	NFULFLD
		clear	NFULCOMP
   	endif
...
NCRCEXIT MOVE      "0" TO NFIELD4
         ...
         ADD       C1 TO SPCOUNT 
        DISPLAY   *P14:20,*EL,"PIDI ORDERS SPOOLED : ",SPCOUNT
...
CreateXMLFile
	...
	pack	str45,"c:\work\pidi.",str8,".t",str6,".jdTst"
.	pack	str45,"\\nts1\e\data\tar.",str8,".t",str6,".jdTst"
	prepare	XMLFile,str45
	write	Xmlfile,seq;"<nin>"
	weof	xmlfile,seq
	close	xmlfile
	clear	str45
	pack	str45,"c:\work\PIDI.",str8,".t",str6,".tst"
.	pack	str45,"\\nts1\e\data\Tar.",str8,".t",str6,".tst"
	clear	XmLFIleName
	pack	XmlFileName,"c:\work\pIDI.",str8,".t",str6,"."
