...
.END PATCH 2.13 ADDED LOGIC
           ...
           MOVE      "APPLY TDMC MERGE INFO" TO STITLE
           CLOCK    DATE TO TODAY
           CALL      PAINT
...
;END PATCH 1.94 REPLACED LOGIC
         move       str35 to user
      Move    "This is a Informational e-mail from  the Merge Info program",SmtpSubject Subject
;   Set the text message that is send with the attachments

       Move    "This is a Informational e-mail from the Merge Info program",SmtpTextMessage(1)   Array <Text message >
        clear   str25
        ...
        Move    " Your above LR had a Merge qty variance:",SmtpTextMessage(3)   Array <Text message >
        clear   str55
...
;         append  CNTNAME,SmtpEmailAddress
;START PATCH 1.97 REPLACED LOGIC
.Patch2.1
				append  "Accounting",SmtpEmailAddress
...
;        Move    CNTNAME,SmtpUserName                                User name
;START PATCH 1.97 REPLACED LOGIC
.Patch2.1
        Move    "Accounting",SmtpUserName                                User name
.        Move    "ComputerRequest",SmtpUserName                                User name
.Patch2.1
;        Move    "InformationServices",SmtpUserName                                User name
;END PATCH 1.94 REPLACED LOGIC
;        Move    STR35,SmtpUserFullName              User Full Name
;START PATCH 1.97 REPLACED LOGIC
         move    "Accounting",SmtpUserFullName              User Full Name
;         move    "InformationServices",SmtpUserFullName              User Full Name

;   Set the destinations of the email. Max 100 (Mime spec)
        Append    user to SmtpDestinations(1,1)                  .send a copy to the originator
        Append    "@nincal.com" to SmtpDestinations(1,1)                  .send a copy to the originator
...
.======================================================================
        ...
        add     eightlpi,row
        add     "60",row
        prtpage laser;*p2000:row,*font=font12,*ALIGNMENT=*Left,*boldon,*ULON,"Triplex Merge Information",*ULOFF,*boldoff;
        add     eightlpi,row
        add     "60",row
...
PrintOpen
			pack printfile,"\\nins1\e\data\NMRGINF.lst"
        	PRTOPEN LASER,"\\NTS0\Laser2","Triplex Merge Printout",noprint,spoolfile=printfile
			return
         INCLUDE   NMRGIO.inc
;patch2.12
	include	compio.inc
