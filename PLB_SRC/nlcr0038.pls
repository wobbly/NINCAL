...
release   init    "3.20"     DLH  Write report in Email body instead of PDF- changed to the default option printflag = 7 to set
REldate   Init      "2015 July xx"    
...
.Release   Init      "3.00"     DLH   if func=1 allow creation of Excel instead of pdf (may just use print switch = 6)
...
PrintFlag form  "00"    .Default is Laser3!!!  01=Laser 3, 02=PDF Email, 03=Laser 2, 04=PDF dump, 05=fax, 06=Excel, 7= HTML in EMAIL BOdy
...
.                    if (PrintFlag = 4 | PrintFlag = 2 | PrintFlag = 6)
                    if (PrintFlag = 4 | PrintFlag = 2 | PrintFlag = 6 | PrintFlag = 7)
.end patch 3.2
.end patch 3.0
...
.break on it.
                        ...
                        if (FIRST = NO)
.                                if          (pdfflag <> "0")          .if HTML don't mess with it
...
.        if (PrintFlag = 2 | PrintFlag = 4 | PrintFlag = 6)
        if (PrintFlag = 2 | PrintFlag = 4 | PrintFlag = 6  | PrintFlag = 7)
.end patch 3.2
.end patch 3.0
...........................................
                    if (PDFFlag <> C1)  .Not using PDF995 to create Email
...
.        elseif (PrintFlag = 2 | PrintFlag = 4)
        elseif (PrintFlag = 2 | PrintFlag = 4 | PrintFlag = 7)
.end patch 3.20
                close   input2
                move    "                                        ",APIFileName
                clear   APIFileName
...
.          Move      "360",MailTimer
          Move      "180",MailTimer
.begin patch 3.10
            move      c1,MailType         .force e-mail HTML body  message type
...
.dave goes bad
        Trap SPOOL1 giving error if SPOOL
.begin patch 3.20  Default No more printing
...
.           append     "<tr>",mailbody
.           append     "<td width=#"40%#" valign=#"top#"><b>TO##: ",mailbody
.           append     OWNLONM,mailbody
.           append      "<br>",mailbody
.           append     COMPHOLD,mailbody
.           append      "<br>",mailbody
.           append     OWNLOSA,mailbody
.           call    Trim using OWNLOCTY
.           if (OWNLOCTY <> "")
.                      pack    taskname,OWNLOCTY,COMMA,OWNLOS,B1,OWNLOZC
.                      append      "<br>",mailbody
...
.           append     "</td>",mailbody
..NIN contact section 3 of table
.           append     "<td width=#"40%#" valign=#"top#"><b>From: ",mailbody
.           append     CNTNAME,Mailbody
.           append     "<br>",mailbody
.           append     "DID: ",mailbody
...
.           append     "FAX: ",mailbody   
.                      if (CNTTEAM = "05") .List Management
.                                 Append     "(415) 945-9451",mailbody
.                      else
.                                 Append     "(415) 433-7796",mailbody
.                      endif
.           append     "<br>",mailbody
.           append     "<br>",mailbody
.           Append     "DATE:  ",mailbody
.           append     newdate1,mailbody
.           append     "</b></td>",mailbody
.           append     "</tr>",mailbody
.           append     "</tbody>",mailbody
...
.detail table 4 sections
           ...
           if         (OSTAT = "l" | OSTAT = "z")
                if (PDFFlag = c1 & (NORD5STAT = "02" OR NORD5STAT = "03"))
