...
PC      EQU     1
Release   INit      "2.5"     AH           .Removed TYPOUT logic - placed it in NADJ0001 instead
REldate   Init      "15 November 2013"
.Release   INit      "2.4"     AH           .Adjusted TYPOUT logic
.REldate   Init      "13 November 2013"
.Release        INit           "2.3"           DLH new code for adjustment Typist
.Reldate        Init           "2013 October 07"
.Release        INit           "2.2"           DLH Check for 64 Bit os
.Reldate        Init           "29 August 2011"
.Release        INit           "2.1"           DLH Turn PL / NIN seperation. Remove CEXTCD = "I" form ncsh001a.plf
..so other code should be fine as transfer code will not be triggered and force company to NIN
.Reldate        Init           "12 November 2009"
.Release        INit           "2.03"           JD Force PL lstmgt list for PL control.
.Release        INit           "2.02"          JD Intercompany code updated again
.Release        INit           "2.01"          JD Intercompany code updated again
.Release        INit           "2.00"          JD Intercompany code updated
.Reldate        Init           "20 October 2008"
...
ReportGo1
        ...
        if (RptCan = NO)
                ...
                clear   taskname
.turned off 04 Jan 2013
.                IF  (Bit64Flag = No)    .if not 64 bit we can submit to batch else run locally
.                append  "\\nins1\lanbatch\batch32 -X -SA -Q\\nins1\ServerA ",TASKNAME
.                 Endif   
.                append  "c:\progra~1\lanbatch\batch -X -SA -Q\\nts0\c\lanbat~2  f:\apps\winbatch\BUTIL.exe ",TASKNAME
                ...
                append  "!\\nins1\winbatch\BUTIL.exe ",TASKNAME
...
.begin patch 1.97 do we  have required comp info????
          call      debug
.begin patch 2.1
.just force it leaving the rest of the code intact
          setitem   CashCompBox,0,C2
.end patch 2.1
...
.END PATCH 1.4 ADDED LOGIC
                if (NewFlag = YES)
                        call    CashUpdateCID
                        if (N2 >= 98)
.                        if (N2 >= 99)        DH temp tp close listworks orders at zero
                                pack    str55 from "Unable to create record.  Contact I.S.!, N2 = ",n2
...
.begin patch 1.97
         ...
         call    NCTRKEY
.begin patch 2.1
.this code should be fine as is
.end patch 2.1
...
.END PATCH 1.9 ADDED LOGIC
.begin patch 1.97
.begin patch 2.1 - this could should be fine as is
...
.begin patch 2.1
.leave the code just going to skip it
          Goto      GoodOne
.end patch 2.1          
..dave goes bad
.         if        (OcompID <> NCTRComp)
...
.                endif
.        call    NTYPUPD
..START PATCH 2.4 REMOVED LOGIC
..        endif
..END PATCH 2.4 REMOVED LOGIC
.AdjUpdateTypistTableEnd
.        return
.
..START PATCH 2.4 REPLACED LOGIC
..NONINE
..          Move      "Ncsh0001 - output",MailSubjct
