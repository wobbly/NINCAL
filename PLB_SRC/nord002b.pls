...
release   init    "10.11"    DLH   New fax gateway
REldate   Init      "2015 April 14"                          .suppress writing to sample list file if hotprint
.release   init    "10.1"    DLH   Bump email var to 100
.REldate   Init      "2014 April 1"                          .suppress writing to sample list file if hotprint
.release   init    "10.0"    ASH  Adjustments to allow Datacard Select items on Order to remain independent from current Datacard state
.REldate   Init      "09 December 2010"                          .suppress writing to sample list file if hotprint
.release   init    "9.9"    DLH  tdmc addy
.REldate   Init      "09 December 2010"                          .suppress writing to sample list file if hotprint
...
.end patch 9.73   .allow restart with file move etc
          ...
          match     "NORD002B",PROGRAM   .case sensitive
          if not equal
                    move      "NORD002B",PROGRAM
...
.first kill from del folder  \\nins1\d\data\orders\delete
.begin patch 9.72
          clear     Mailbody
          FIndDIr   "\\nins1\d\data\orders\delete\*.*",MailBody,Itemcount=n5
          if        (n5 > c0)
          ...
          match     "f",Dmfilename 
                    ...
                    clear     taskname
                    bump      DmFileName,c1
                    pack      taskname from "\\nins1\d\data\orders\delete\",DmFIleName  ."comment :)
                    FindFIle  Taskname
...
.         Destroy   DLFIles
.end patch 9.72
.second move all from old to del   \\nins1\d\data\orders\old
.begin patch 9.72
          clear     Mailbody
          FIndDIr   "\\nins1\d\data\orders\old\*.*",MailBody,Itemcount=n5
          if        (n5 > c0)
          FOr       n4 from c0 to N5
          explode   MailBody,"|",Dmfilename 
          match     "f",Dmfilename 
                    ...
                    clear     taskname
                    bump      DmFileName,c1
                    pack      str55 from "\\nins1\d\data\orders\old\",DmFIleName           ."comment :)
                    pack      taskname from "\\nins1\d\data\orders\delete\",DmFIleName  ."comment :)
                    FindFIle  Str55
                              if        zero                .file is there
...
.         dlFiles.GetText giving dmFileName using *Index=DLndx
.         clear     taskname
.         pack      str55 from "\\nins1\d\data\orders\old\",DmFIleName
.         pack      taskname from "\\nins1\d\data\orders\delete\",DmFIlename
.         
.               rename          Str55,taskname
.         repeat
.         Destroy   DLFIles
.end patch 9.72
.last move from sent to old  \\nins1\d\data\orders\sent
.begin patch 9.72
          clear     Mailbody
          FIndDIr   "\\nins1\d\data\orders\sent\*.*",MailBody,Itemcount=n5
          if        (n5 > c0)
          FOr       n4 from c0 to N5
          explode   MailBody,"|",Dmfilename 
          match     "f",Dmfilename 
                    ...
                    clear     taskname
                    bump      DmFileName,c1
                    pack      str55 from "\\nins1\d\data\orders\Sent\",DmFIleName     ."comment :)
                    pack      taskname from "\\nins1\d\data\orders\old\",DmFIleName    ."comment :)
                    FindFIle  Str55
                              if        zero                .file is there
                              Rename    Str55,Taskname                          
...
.         dlFiles.GetText giving dmFileName using *Index=DLndx
.         clear     taskname
.         pack      STr55 from "\\nins1\d\data\orders\sent\",DmFIleName
.         pack      taskname from "\\nins1\d\data\orders\old\",DmFIlename
.         
.         FINDFILE Taskname,WRITE=Str25
.               rename        str55,Taskname
...
.......................................
          ...
          IF        (EmailFlag1 = yes)              .email
                    IF        (formflag = 2 | formflag = 3)          .Owner / Fulfillment 
                              ...
                              append    CRLF,Mailbody
                              append    "180 Grand Avenue, Suite 1365           180 Grand Avenue, Suite 1365",Mailbody
                              append    CRLF,Mailbody
                              append    "Oakland, CA 94612-3716                           Oakland, CA 94612-3716",Mailbody
                              append    CRLF,Mailbody
                    ...
                    Else
                              ...
                              append    CRLF,Mailbody
                              append    "180 Grand Avenue, Suite 1365",Mailbody
                              append    CRLF,Mailbody
...
OPRINT3
          ...
          loop
                    ...
                    if (NSEL3CODE = "A")
                              ...
                              call      NADDKEY
.START PATCH 10.0 REPLACED LOGIC
.                              if not over
...
.START PATCH 10.0 REMOVED LOGIC
.                              endif
.END PATCH 10.0 REMOVED LOGIC
                    elseif (NSEL3CODE = "L")
