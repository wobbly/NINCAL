...
.
         ...
         include   nmlddd.inc
          include   Commdd.inc
.QualMlrs  Init      "7111-7117-7416-2532-5885-7124-7391-7450-2405-7489-8624-1531-8224-7436-9982-7433-8501-3021-7487"
QualMlrs  Init      "7111-7117-7416-2532-5885-7124-7391-7450-2405-7489-8624-1531-8224-7436"
QualMlrs1 Init      "7529-4192-9234-9856-2006-7628-2364-7544-9982-7433-8501-3021-7487-4090-7665-1866"
.Jan 2011

.7529   Wistar ordered 9/16 – they’ve not paid yet, but if they do pay on time they should be included.
.4192   Project Angel Food ordered 9/15 this should be on
.9234   Bay Citizen ordered 10/15 this too
.9856   United Cerebral Palsy of Southern California ordered 10/28 this too
.2006   Lewis and Clark Humane Society ordered 11/11 this too
.7628   SAFE Haven for Cats ordered 11/30 on – yes – though not paid
.2364   KOCE ordered 12/23 yes, though not paid (billed either?)
.7544   WYCC-TV20 ordered 12/9 yes though not paid

.end Jan 2011
.Boston Medflight – 7111/10364
.California Institute of Integral Studies – 7117/10369
.Community Action Board of Santa Cruz County – 7416/10435
.Earth Island – 2532/2471
.Florence Crittenton Services - 5885/5460
.St. Anthony Foundation – 7124/10378
.Institute for Population Studies – 7391/10413
.Planned Parenthood Southern New England -7450/10471
.Farm Animal Rights Movement – 2405/2390
.The Wilderness Land Trust – 7489/10511
.Larkin Street Youth – over 1 year rule – 8624/7507
.The Xerces Society – over 1 year rule – 2015/9591   .... SA says no
.New Century Chamber Orchestra – over 1 year rule – 1531/9354
.LYRIC – over 1 year rule – 6670/6188   .... SA says no
.Coral Reef Alliance – 8224/7140
.600 Million Stray Dogs -7436/10454
.Oregon Human Society – 9982/18830
.Romanian League in Defense of Animals – 7433/10541
.International Animal Rescue – 8501/7391
.FRICK ART         3021/9933           added 9/2/10
.Reef Check         7487/10509           added 9/2/10
.have started a new database Commission.dat
.salesperson #
.Company #
.Start date of comm
.company name
...
.release   init      "1.0"          DLH     .New for Schoevaars
.reldate   Init      "7 June 2010"
.CLOCK    FUNCTION
........................
DATE      DIM       10
SYSMO     DIM       2
...
. .............................................................................
         ...
         MOVE      "NINC0005" TO PROGRAM
         MOVE      "Commission INCOME " TO STITLE
...
.
         PACK      STR35,NTWKPATH1,"INCOME5"
         PACK      STR45,NTWKPATH1,"INCOME5"
         
          Prepare   Output,"c:\work\income5.tmp",exclusive   
          PREPARE   OUTPUT1,STR35,STR45,"8","290",exclusive   
          Prepare   CSVFIle,"c:\work\income5.CSV",exclusive   
          write     CsvFile,seq;*cdfon,"order ID","order qty","Exch Qty","est Rev","est INC","Order date","qty bild","Revenue","Income":
                    ...
                    "Days to Pay","Qualified","Mlr ID","Mlr Name","Division"                    ."
...
...........................Load table here
          call      Debug
         Display    *P10:12,"Loading Commission Tables"
          ...
          if        (COMMsls = "029")
          add       c1,COMmRecCount
          MOVe      Commcpy,Commrec(COMMRecCount).CommNumREc
          MOVe      CommDate,Commrec(COMMRecCount).CommStartREc
          unpack    commdate into str2,yy,mm,dd
          call      cvtjul
          add       "365" to juldays
          call      cvtgreg
          pack      str8 from cc,yy,mm,dd
          MOVe      str8,n8
          move      N8,Commrec(COMMRecCount).CommSTopREc        
          endif
          Move      CommRecCount,RecCount
          repeat
...
.................
          ...
          call      Nordks
          until     over
         Display    *P10:14,"Reading orders, LR: ",olrn," Order Date: ",oodtem,slash,oodted,slash,oodtec,oodtey
...
.  for Shirley - check for shipdate greater than 12/1         
.if so goto skip, if not ship info check for return date
          MOVE      Olrn to nshpfld
          REP       ZFILL IN NshpFLD
          call      Nshpkey
          if        Not over
          rep       zfill in sdate
          move      Sdate to N8              .sdate is in format of ccyymmdd
                    if        (N8 > "20111201")
                    goto      Skip
                    endif
          Else                               .it was over check return date
          pack      str8 from ORTNDTEC,ORTNDTEY,ORTNDTEM,ORTNDTED
          rep       zfill in str8
          move      str8,n8
                    if        (N8 > "20111201")
                    goto      Skip
...
.....update this logic to use data from Commissions.dat (loaded into table with sales person# compcode, start and end dates
          ...
          call      NMLRKEY
          Move      No,OnTheList
          FOR       CommRecCount,"1",RecCount
          MOVe      Commrec(COMMRecCount).CommNumREc,Str6
                    if        (compnum = str6)                        .its a hit
                    MOVe      Commrec(COMMRecCount).CommStartREc,StartDate
                    move      Commrec(COMMRecCount).CommSTopREc,StopDate        
                    Move      Yes,OnTheList
                    Break
                    endif                    
          Repeat
...
.          reset     QUalMlrs
.          reset     QUalMlrs1
.          scan      Omlrnum in QualMLrs
.                    if Not equal
.                    scan      Omlrnum in QualMLrs1
.                    goto      Skip if not equal
.                    endif
                    
                                        if        (n8 >= "20090701")
.                                        if        (n8 >= "20090701" & N8 <= "20091231")
                                        call      write
                                        endif
Skip
          repeat
...
Pass2
          Open      Output,"c:\work\income5.tmp",exclusive
...
.....update this logic to use data from Commissions.dat (loaded into table with sales person# compcode, start and end dates
          ...
          Repeat
           ...
           move      str8 to n8
           if        (StartDate <= N8 & N8 <= StopDate)
           call     debug         
           Move     Yes,qualified
.add code for days to pay           
           else
           Move     No,qualified
           endif
...........................
...
WriteCsv
          ...
          write     CsvFile,seq;*cdfon,Olrn,oqty,NotXnames,ungross,unbilinc,Odate,c0,c0,c0,Bdate,MLdate,c0,c0,c0,c0,c0,c0:
                    Pdate,DaystoPay,Qualified,outid,name,str35
...
.===========================================================================
          Weof      CsvFIle,seq
          Close     Csvfile
          OPen      OUTPUT1,"\\nins1\e\data\income5",exclusive   

.begin patch 1.10
                              PRTOPEN PrFile,"PDF:","c:\work\pdf\Income5.pdf",Flags=PDF_FLAGS_WIN_ANSI_ENCODING
.                              PRTOPEN PrFile,"PDF995","Income5"
.                              PRTCLose Prfile
