...
INVFLAG  DIM       1         
.
.....>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>for "Monday" numbers - needs new biz logic
Release  Init      "4.65"     DLH emails to HTML and office365 server, and internal sort
RelDate  INit      "2015 December 30"
.Release  Init      "4.64"     DLH include NIN income
.RelDate  INit      "2015 February 17"
.Release  Init      "4.63"     DLH - clarification for "Monday" numbers - needs new biz logic
...
BEGIN 
         ...
         SETLPTR   MONTH
          ...
          Move      c3,Ninvlock
.
         DISPLAY   *P10:09,"Sorting out current Adjustments: "
.RunWait('%sort%','\\nins1\e\data\text\nadjust.dat,\\nins1\e\data\Nadjust.eom /inc(127,2,n,eq,\"%yr%\",and,129,2,n,eq,\"%mo%\") w(c:) ver')
.RunWait("%sort%","\\nins1\e\data\Nadjust.eom,\\nins1\e\data\adjust.eom /s(127,8,n,a,119,6,n,a,141,2,n,a) w(c:) ver")
SortFile
          Erase     "\\nins1\e\data\nadjust.eom"   
          Pack      Taskname from "\\nins1\e\data\text\Nadjust.dat,\\nins1\e\data\nadjust.eom;S=#"127=#'",yy,"#'&129=#'",mm,"#'#""
          Sort      Taskname,SunDM="NINS1:502"
                    If        Over
                              pack      str255 from "Sort to nadjust.eom failed, ",CRLF,"Error: ",S$ERROR$,crlf:
...
SortFile1
          Erase     "\\nins1\e\data\adjust.eom"   
          Pack      Taskname from "\\nins1\e\data\Nadjust.Eom,\\nins1\e\data\adjust.eom;114-115,127-132,119-125,141-142"
          Sort      Taskname,SunDM="NINS1:502"
                    If        Over
                              pack      str255 from "Sort to adjust.eom failed, ",CRLF,"Error: ",S$ERROR$,crlf:
                              "Yes = you fixed, No=try again, Canncel= abort"                 
                              ALERT    PLAIN,str255,RESULT
                              IF       (RESULT = 1)
                                        ALERT    NOTE,"YES was pressed.",RESULT
...
.              MOVE      INPNAME TO NJSTNAMESEQ    
          pack      NJSTNAMESEQ from INPName,"|NINS1:502"
              
                      
         CALL      HEADING
         CLEAR     STR55
           pack       str4 using yy,mm
           rep        zfill,str4
...
.>Patch 3.8 Code Modified         
         ...
         RESET     STR55
         PREPARE   DETOUT,STR55,exclusive
         MOVE      C1 TO NINVPATH
.
RDADJ    CALL      NJSTSEQ
         GOTO      EOJ IF OVER
...
.         if        equal
         ...
         write     detout,seq;*cdfon,"Mailer","LR","Invoice","adj number":
                   "A/R","A/P1","A/P2","LRInc","Adj Date","adj code","List","Reason","Division"
.                   "A/R","A/P1","A/P2","LRInc","NInc","Adj Date","adj code","List","Reason"
         endif           
         write     detout,seq;*cdfon,jstmlr,jstlr,jstinvno,jstsubno:
                   FART,FAP1T,FAP2T,LRINC,NINC,jstdate,JSTCD,O1DES,jstreasn,DivFlag
