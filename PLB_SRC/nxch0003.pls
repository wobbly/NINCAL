...
PC       EQU       0
         ...
         INCLUDE   NOFRDD.INC
          ...
          include   NXNGDD.INC
          INCLUDE   NXCHDD.INC
release   init      "4.33"                      DLH       .Summary report if transaction date was selected it was printing 2x
reldate   init      "2015 May 6"
.release   init      "4.32"                      DLH       .allow inhouse and other options with excel summmary
.reldate   init      "2014 August 07"
.release   init      "4.31"                      DLH       bug fix
...
TRANflag form      1                  1=no transaction date 2=transaction date
             ...
             call      GetWinVer
                    ...
                    if (PrintFlag = C0 | PrintFlag = 1)     .Laser2 
                          ...
                          elseif (PrintFlag = 2)  .Laser3
                              if (osflag >= c6)
                                          PRTOPEN Laser,"\\nins2\Laser3 Blankstock",prtname
                                  else   .(osflag = c0)         .Don't know prompt for printer
                                          PRTOPEN Laser,"-",prtname
                                  endif
...
.          pack      Exrange2 from "C",DimRow2
          ...
          call      Trim using DimRow1                 
          pack      Exrange1 from "A",DimRow1
          pack      str55 from "©1991-2016, Names in the News"
...
CheckFile
          ...
          trap      IOMssg GIVING ERROR IF IO
           ...
           if         (Excelflag = 2)
          Move      "Here is your Excel Exchange Report",MailSubjct
           else
          Move      "Here is your PDF Exchange Report",MailSubjct
          endif
          Clear     MailBody
...
.end patch 4.2
          Elseif   (excelFlag = c2)
                    ...
                    pack            Exrange1 from "F",DimRow1
                    setprop sheet.range(ExRange1,ExRange1),*Value=Date,*HorizontalAlignment=XLAlignCenter
.check universe and other flags, include as requested
...
DETAILA
         ...
         CLEAR     STATMSG
         CMATCH    "X" TO STAT
         GOTO      PRCANCL1 IF EQUAL                 .ORDER CANCELLED  Not adjusted
.                                                    AFTER MAILDATE
         GOTO      DETAILC
.
DETAILB
...
.         call       Newpage
          ...
          add       sixlpi,row
.begin Patch 4.32
           if         (stat = "C" | stat = "R")                .cancelled and adjusted or changed to rental
           call       trim using xchcomnt
           pack       xchcomnt from xchcomnt,b1,"qty was: ",qtyout
           move       c0,qtyout
           endif
.end Patch 4.32
...
.                    *p=7750:row,Str10,*p=8000:row,XCHCOMnt        .move comments to the next line
                    *p=7750:row,Str10
...
.                      if         (Flag = "I")
.                      setprop sheet.range(ExRange1,ExRange1),*Value="Inactive"
.                      endif
          endif          
.
         ADD       C1,LINES
...
.                   call       Newpage
          ...
          prtpage   Laser;*p=10:row,OMlrpon,*p=1500:row,OFDESC:
                    *p=5400:row,NSEL2Name,*p=8000:row,XCHCOMnt          .move comment here
...
.begin Patch 4.32                
                              ...
                              call            Trim using DimRow1                 
                              pack            Exrange1 from "A",DimRow1
                              setprop sheet.range(ExRange1,ExRange1),*Value=nxrflist,*NumberFormat="#####0"
                                 endif
.end Patch 4.32                  
.begin Patch 4.32  --- maybe correct spot?
.Excel not supported yet
                                 if         (univFlag = c2)
                              Move            Row1,DimRow1
                              call            Trim using DimRow1                 
                              pack            Exrange1 from "F",DimRow1
                              setprop sheet.range(ExRange1,ExRange1),*Value=N10,*NumberFormat="##,####0"
                                 endif

                              Move            Row1,DimRow1
                              call            Trim using DimRow1                 
                              pack            Exrange1 from "G",DimRow1
                              setprop sheet.range(ExRange1,ExRange1),*Value=str10,*NumberFormat="m/d/yyyy"

                                 if         (tranflag = c2)
                              Move            Row1,DimRow1
                              call            Trim using DimRow1                 
                              pack            Exrange1 from "H",DimRow1
                              setprop sheet.range(ExRange1,ExRange1),*Value=Date,*NumberFormat="m/d/yyyy"
                                 endif
.end Patch 4.32
...
OH1
         COMPARE   C0 TO PAGE
         IF        EQUAL
.begin patch 4.31    missing excel check added
                                          IF        (excelFlag = c1)                   
                                            prtpage   Laser;*UNITS=*HIENGLISH:
                                         *ORIENT=*Portrait,*Font=Font1
                                         endif
.end patch 4.31    missing excel check added
           ADD       C1 TO PAGE
                   ELSE
         CALL      PAGENUM
         ENDIF
...
.          pack      Exrange1 from "B",DimRow1
          ...
          call      Trim using DimRow1                 
.begin Patch 4.32
           if         (listflag = c2)                  .inhouse
          pack      Exrange1 from "A",DimRow1
          setprop sheet.range(ExRange1,ExRange1),*Value="List ##"
          setprop sheet.range(ExRange1,ExRange1).Font,*Bold="True"
           endif
.end Patch 4.32
.          pack      Exrange1 from "C",DimRow1
...
.          pack      Exrange1 from "D",DimRow1
          ...
          setprop sheet.range(ExRange1,ExRange1).Font,*Underline=xlUnderlineStyleSingle
.begin Patch 4.32
           if         (Tranflag = c2)                  .last transaction wanted
          pack      Exrange1 from "H",DimRow1
          setprop sheet.range(ExRange1,ExRange1),*Value="Last Use"
          setprop sheet.range(ExRange1,ExRange1).Font,*Bold="True"
           endif

           if         (univflag = c2)                  .Universe wanted
          pack      Exrange1 from "F",DimRow1
          setprop sheet.range(ExRange1,ExRange1),*Value="Universe"
          setprop sheet.range(ExRange1,ExRange1).Font,*Bold="True"
           endif

           if         (RevvFlag = yes)                 .Last Update wanted
          pack      Exrange1 from "G",DimRow1
          setprop sheet.range(ExRange1,ExRange1),*Value="List Update"
          setprop sheet.range(ExRange1,ExRange1).Font,*Bold="True"
           endif
...
pagemask
         ...
         If        ("T" = prtsw & tranflag = "2")         
                    IF        (excelFlag = c1)
                          Move   "10500",row           
                    prtpage   laser;*p100:row," ©1991-2016, Names in the News":
           ...
           if        equal
                      IF        (excelFlag = c1)
                                 Move       "8000",row 
                      prtpage   laser;*p100:row," ©1995-2016, Names in the News":
